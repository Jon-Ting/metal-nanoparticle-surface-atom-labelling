...
def calculate_dc(distance_matrix, percentage=1.2):
    '''
    This function is to calculate the cut-off distance for calculation of the 2 metrics (rho and delta).
    INPUTS:
        distance_matrix: N * N matrix (calculated by pairwise_distance)
        percentage: choose the percentage of the nearest point around each point of the total
    OUTPUTS:
        dc: cut-off distance
     '''
    temp = []
...
def entropy(density):
    '''
    Calculate the data field entropy. (pg. 399 of Ref. 2)
    INPUTS:
        density: density of N points
    OUTPUTS:
        H: the entropy (uncertainty) of the current data point
    '''
    Z = np.sum(density)  # Normalisation factor
    temp = []
    for i in density:
        faiz = i/Z
...
def continuous_density(distance_matrix, dc):
    '''
    Measure the density of a point using Gaussian kernel
    INPUTS:
        distance_matrix: N * N matrix (calculated by pairwise_distance)
        dc: cut-off distance
    OUTPUTS:
...
def discrete_density(distance_matrix, dc):
    ...
    density = np.zeros(shape=len(distance_matrix))
    for index, dis in enumerate(distance_matrix):
        # Number of points larger than the cut-off distance
        density[index] = len(dis[dis < dc])
    return density

def choose_dc(distance_matrix):
    '''
    Non-parametric way of choosing the cut-off distance
    INPUTS:
        distance_matrix: N * N matrix (calculated by pairwise_distance)
    OUTPUTS:
        dc: cut-off distance
...
def delta_function(distance_matrix, density):
    ...
    INPUTS:
        ...
        density: density of N points
    OUTPUTS:
        delta_matrix: minimum distance between the point and any other with high density
        closest_point: the closest point with higher density than the current point
    '''
...
def applyILS(X, index):
    '''
    Combined ILS with the centroid finding
    INPUTS:
    ...
    df1 = X.copy()
    df1['label'] = 0
    count = 1  # Label for each cluster
    for i in index:
...
def draw_ILS_with_shape(count, X_embedded, newL, colors, original_df):
    '''
    plot the 2D results, labelled by the 'shape' feature
    INPUTS:
        count: cluster number+1
        X_embedded: 2D dataframe, column name 'x' and 'y'
        newL: new labels
...
def top_k_idx(gamma, k):
    '''
    Sort the gamma list and get the top k values' indexes of the list
    INPUTS:
        gamma: the product of the density value and the minimum distance value at each point
        k: the top k number
    OUTPUT:
...
def plot_centroid(X_embedded, index):
    '''
    Plot the centroid chosen from the gamma graph
    INPUTS:
        X_embedded: 2D dataframe, column name 'x' and 'y'
        index: the index list of the top k gamma values
    '''
