      program SYMMOL
      implicit double precision (a-h,o-z)
c logical exists
      PARAMETER (NMA=1000)
      CHARACTER*2 ATOM(NMA)
      CHARACTER*80 LINE
c      CHARACTER*60 inpfile
c      CHARACTER*60 outfile
      common/matcel/PC(7),PCR(7),O(3,3),OI(3,3),G(3,3),GI(3,3),CS(12)
      COMMON/AT0/NA,NMO
      COMMON/AT1/X(3,NMA),AMAS(NMA),RCOV(NMA),MSP(NMA)
      COMMON/AT2/SX(3,NMA),SIG(NMA),DXM(NMA),DCM,DCME,indwgh,indtol
      COMMON/AT3/MOL(NMA),MLG(NMA)
      CHARACTER*6 NAME,SPEC
      COMMON/AT4/NAME(NMA)
c________________________________________________________________________
c      open(7,file=outfile)
      open(7,file='symmol.out')
      write(7,*)'                  ============'
      write(7,*)'                     SYMMOL'
      write(7,*)' A PROGRAM FOR THE SYMMETRIZATION OF GROUPS OF ATOMS'
      write(7,*)'       By Tullio Pilati and Alessandra Forni'
      write(7,*)'               Version November 4th 2002'
      write(7,*)' ==================================================='
      write(7,*)
c  900 write(7,*)'input file name?'
c      read(*,'(a)')inpfile
c      inquire(file=inpfile,exist=esiste)
c      if(esiste)then
c         open(1,file=inpfile)
c      else
c         write(7,'(a60,a)')inpfile,' does not exist'
c         stop
c      endif

c      read(*,'(a)')inpfile
c      open(1,file=inpfile)
      open(1,file='symmol.inp')
 1000 read(1,'(A80)')LINE
      if(LINE(1:1).eq.'#') go to 1000
      READ(LINE,*)(PC(i),i=1,6)
c********************************************
 1010 read(1,'(A80)')LINE
      if(LINE(1:1).eq.'#') go to 1010
      READ(LINE,*)indwgh,indtol,DCM,DCME
      if(DCME.lt.DCM)DCME=DCM
      write(7,*)
      if(indwgh.le.1)then
      indwgh=1
      write(7,*)' INDWGH=1 ===> WEIGHTS AS ATOMIC AMASS'
      endif
      if(indwgh.eq.2)write(7,*)' INDWGH=2 ===> UNITARY WEIGHTS'
      if(indwgh.eq.3)write(7,*)' INDWGH=3 ===> WEIGHTS BASED ON S.U.'
      if(indtol.le.1)then
      indtol=1
      write(7,*)' INDTOL=1 ===> TOLERANCE=CONSTANT'
      endif
      if(indtol.eq.2)write(7,*)
     *               ' INDTOL=2 ===> TOLERANCE BASED ON DISTANCES'
      if(indtol.eq.3)
     *         write(7,*)' INDTOL=3 ===> TOLERANCE BASED ON S.U.'
      write(7,1)DCM,DCME
    1 format('  CONSTANTS OF TOLERANCE=',2f7.3)
      write(7,*)
c********************************************
      i=1
      MOmax=0
 1100 read(1,'(A80)',END=1200)LINE
      if(i.gt.NMA)then
        write(7,*)'ERROR: TOO MANY ATOMS IN INPUT'
        write(7,*)'Enlarge PARAMETER NMA'
        write(7,*)'Actual value: PARAMETER (NMA=1000)'
        stop
      endif
c     write(7,*)line
      if(LINE(1:1).eq.'#')go to 1100
      READ(LINE,'(A6,I2,6f9.5)')NAME(i),MOL(i),(X(k,i),k=1,3),
     *(SX(k,i),k=1,3)
      if(MOL(i).gt.MOmax)Momax=MOL(i)
      ATOM(i)='  '
      SPEC=NAME(i)
      call COMPACT(SPEC,6,K)
      K=1
      if(SPEC(2:2).ge.'A'.and.SPEC(2:2).le.'Z')K=2
      if(SPEC(2:2).ge.'a'.and.SPEC(2:2).le.'z')K=2
      call CAPITAL(SPEC(1:1))
      IF(K .GT. 1) then
      call CAPITAL(SPEC(2:2))
      ATOM(i)=SPEC(1:2)
      else
      ATOM(i)(2:2)=SPEC(1:1)
      endif
      i=i+1
      go to 1100
 1200 NA=i-1
      if(indwgh.eq.3.or.indtol.eq.3) then
        sxmax=0.000001d0
        do i=1,NA
            do k=1,3
              if(sx(k,i).gt.sxmax) sxmax=sx(k,i)
            enddo
        enddo
        if(sxmax.lt.0.00001d0) then
          write(7,*)
          write(7,*) ' ERROR: S.U. ON COORDINATES ARE MISSING.'
          if(indtol.eq.3)
     *       write(7,*) '       INTRODUCE THE S.U. OR CHANGE INDTOL'
          if(indwgh.eq.3)
     *       write(7,*) '       INTRODUCE THE S.U. OR CHANGE INDWGH'
          stop
        endif
      endif
      close(1)
C
      call mass(NA,ATOM)
c     call constants
      call cella
      write(7,*)
      write(7,*)' ATOM GROUP  INPUT COORDINATES AND THEIR S.U.'
      write(7,*)
      do I=1,NA
      write(7,'(1x,A6,i2,1x,6f9.5,2x,i2)')NAME(I),MOL(I)
     *,(X(k,I),k=1,3),(SX(k,I),k=1,3)
      enddo
C
      do 1300 MO=1,MOmax
      call work(MO)
 1300 continue
      end
      subroutine arccos(cost,ang)
C arc cosine in degrees
      implicit double precision (a-h,o-z)
      RAD=57.29577951308232D0
      if(cost.gt.1.d0)cost=1.d0
      if(cost.lt.-1.d0)cost=-1.d0
      ang=RAD*DACOS(cost)
      return
      end
      subroutine asymunit(MK,IASU,N,NMS)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      DIMENSION MK(NMA,NMG),IASU(NMA)
      do i =1,N
        IASU(I)=2
      enddo
      DO 2800 I=1,N
        if(IASU(I).ne.2)go to 2800
      DO 2790 J=1,NMS
      K=IABS(MK(I,J))
      if(K.eq.I)go to 2790
      if(K.eq.0)then
        IASU(I)=0
        go to 2800
      endif
        IASU(K)=1
 2790 CONTINUE
 2800 CONTINUE
      return
      end
      subroutine ax_order(A,i,m,msign,invers)
C
C m = group order for the matrix SIM(i)
C msign= 1 own rotation axis
C msign=-1 improper axis of rotation
C     
      PARAMETER (maxorder=8)
      implicit double precision (a-h,o-z)
      dimension A(3,3,*),B(3,3),C(3,3,2)
      call reset(C,0.d0,18)
      C(1,1,1)=1.d0
      C(2,2,1)=1.d0
      C(3,3,1)=1.d0
      C(1,1,2)=-1.d0
      C(2,2,2)=-1.d0
      C(3,3,2)=-1.d0
      invers=2
      msign=NINT(det(A,i))
        call prodmm(A,C,B,i,1,1)
      do m=1,2*maxorder
        if(ium(C,B,1.d-2,2,1).eq.1)invers=1
        if(ium(C,B,1.d-2,1,1).eq.1)go to 1000
        call prodmm(A,B,B,i,1,1)
      enddo
      write(7,*)'INPUT PARAMETER DCM probably too HIGH. Reduce it!'
      stop
1000  if(m.lt.6.or.msign.eq.1)return
      m1=(m/4)*4
      if(m1.ne.m)m=m/invers
c     write(7,*)'ax_order,i,m,msign,invers'
c     write(7,'(10i5)')i,m,msign,invers
c     write(7,'(3f10.6)')((A(ii,kk,i),kk=1,3),ii=1,3)
      return
      end
      subroutine reset(A,COST,N)
      implicit double precision (a-h,o-z)
C resetMENTO
      DIMENSION A(1)
      DO 1 I=1,N
    1 A(I)=COST
      RETURN
      END
      subroutine bond(XO,IASU,N)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      COMMON/AT1/X(3,NMA),AMAS(NMA),RCOV(NMA),MSP(NMA)
      CHARACTER*6 NAME
      COMMON/AT4/NAME(NMA)
      COMMON/AT5/BONDDD(20,NMA),NLEG(NMA)
      DIMENSION XO(3,NMA),IASU(NMA)
      DIMENSION T(3)
      do 1200 i=1,N
      r1=RCOV(i)
      N1=0
      do 1100 k=1,N
      if(IASU(k).eq.0)go to 1100
      if(k.eq.i)go to 1100
      r2=(r1+RCOV(k))*1.2
      call combli(XO,XO,T,1.d0,-1.d0,i,k,1)
      call prods(t,t,d1,1,1,2)
c     write(7,'(a20,2i3,2f10.5)')'i,k,d1,r2',i,k,d1,r2
      if(d1.gt.r2)go to 1100
      N1=N1+1
      if(N1.gt.20)then
        go to 1200
      endif
      BONDDD(N1,I)=k
 1100 continue
      NLEG(i)=N1
 1200 continue
c     write(7,*)'BONDDD'
c     do ii=1,N
c     N1=NLEG(II)
c     write(7,'(20i3)')(BONDDD(kk,ii),kk=1,N1)
c     enddo
      return
      end
      double precision function brent(ax,bx,cx,func,tol,xmin)
      implicit double precision (a-h,o-z)
      INTEGER ITMAX
c     REAL brent,ax,bx,cx,tol,xmin,func,CGOLD,ZEPS
      EXTERNAL func
      PARAMETER (ITMAX=100,CGOLD=.3819660,ZEPS=1.0e-10)
      INTEGER iter
c     REAL a,b,d,e,etemp,fu,fv,fw,fx,p,q,r,tol1,tol2,u,v,w,x,xm
      a=min(ax,cx)
      b=max(ax,cx)
      v=bx
      w=v
      x=v
      e=0.d0
      fx=func(x)
      fv=fx
      fw=fx
      do 11 iter=1,ITMAX
        xm=0.5d0*(a+b)
        tol1=tol*abs(x)+ZEPS
        tol2=2.d0*tol1
        if(abs(x-xm).le.(tol2-.5d0*(b-a))) goto 3
        if(abs(e).gt.tol1) then
          r=(x-w)*(fx-fv)
          q=(x-v)*(fx-fw)
          p=(x-v)*q-(x-w)*r
          q=2.*(q-r)
          if(q.gt.0.d0) p=-p
          q=abs(q)
          etemp=e
          e=d
          if(abs(p).ge.abs(.5*q*etemp).or.p.le.q*(a-x).or.p.ge.q*(b-x)) 
     *goto 1
          d=p/q
          u=x+d
          if(u-a.lt.tol2 .or. b-u.lt.tol2) d=sign(tol1,xm-x)
          goto 2
        endif
1       if(x.ge.xm) then
          e=a-x
        else
          e=b-x
        endif
        d=CGOLD*e
2       if(abs(d).ge.tol1) then
          u=x+d
        else
          u=x+sign(tol1,d)
        endif
        fu=func(u)
        write(7,*)'brent,fu,fx',fu,fx
        if(fu.le.fx) then
          if(u.ge.x) then
            a=x
          else
            b=x
          endif
          v=w
          fv=fw
          w=x
          fw=fx
          x=u
          fx=fu
        else
          if(u.lt.x) then
            a=u
          else
            b=u
          endif
          if(fu.le.fw .or. w.eq.x) then
            v=w
            fv=fw
            w=u
            fw=fu
          else if(fu.le.fv .or. v.eq.x .or. v.eq.w) then
            v=u
            fv=fu
          endif
        endif
11    continue
c     pause 'brent exceed maximum iterations'
3     xmin=x
      brent=fx
      return
      END
      subroutine cella
      implicit double precision (a-h,o-z)
      common/matcel/PC(7),PCR(7),O(3,3),OI(3,3),G(3,3),GI(3,3),CS(12)
      RAD=57.29577951308232D0
      ARAD=1.D0/RAD
      PC(7)=1.D0
      PCR(7)=1.D0
      COM=1.D0
      DO 1030 I=1,3
      K=I+3
      CS(I)=COS(PC(K)*ARAD)
      CS(K)=SIN(PC(K)*ARAD)
      IF(PC(K).GT.0.D0)GO TO 1030
      PC(K)=90.D0
      CS(I)=0.D0
      CS(K)=1.D0
 1030 COM=COM-CS(I)*CS(I)
      COM=SQRT(COM+2.D0*CS(1)*CS(2)*CS(3))
      O(1,1)=PC(1)*CS(6)
      O(1,2)=0.D0
      O(1,3)=PC(3)*(CS(2)-CS(1)*CS(3))/CS(6)
      O(2,1)=PC(1)*CS(3)
      O(2,2)=PC(2)
      O(2,3)=PC(3)*CS(1)
      O(3,1)=0.D0
      O(3,2)=0.D0
      O(3,3)=PC(3)*COM/CS(6)
      IF(ABS(O(2,1)).LT.1.D-10)O(2,1)=0.D0
      IF(ABS(O(2,3)).LT.1.D-10)O(2,3)=0.D0
      IF(ABS(O(1,3)).LT.1.D-10)O(1,3)=0.D0
      PC(7)=det(O,1)
      call traspo(O,G,1,1)
      call prodmm(G,O,G,1,1,1)
      call inv33(O,OI,1,1)
      PCR(7)=det(OI,1)
      call inv33(G,GI,1,1)
      PCR(1)=SQRT(GI(1,1))
      PCR(2)=SQRT(GI(2,2))
      PCR(3)=SQRT(GI(3,3))
      CS(7)=GI(2,3)/(PCR(2)*PCR(3))
      CS(8)=GI(1,3)/(PCR(1)*PCR(3))
      CS(9)=GI(1,2)/(PCR(1)*PCR(2))
      PCR(4)=RAD*DACOS(CS(7))
      PCR(5)=RAD*DACOS(CS(8))
      PCR(6)=RAD*DACOS(CS(9))
      CS(10)=SIN(ARAD*PCR(4))
      CS(11)=SIN(ARAD*PCR(5))
      CS(12)=SIN(ARAD*PCR(6))
      write(7,*)
      write(7,*)' CELL'
      write(7,'(3f10.5,3f10.3,f11.5,/)')PC
c     write(7,*)' RECIPROCAL CELL'
c     write(7,'(3f10.5,3f10.3,f11.5,/)')PCR
c     write(7,*)' METRIC MATRIX'
c     write(7,'(3g16.6)')((G(i,k),k=1,3),i=1,3)
c     write(7,*)
c     write(7,*)' ORTHOGONALIZATION MATRIX'
c     write(7,'(3g16.6)')((O(i,k),k=1,3),i=1,3)
c     write(7,*)
      return
      END
      subroutine combli(A,B,C,D,E,I,J,K)
C linear combination D*A+B*E=C
      implicit double precision (a-h,o-z)
      DIMENSION A(3,1),B(3,1),C(3,1)
      C(1,K)=A(1,I)*D+B(1,J)*E
      C(2,K)=A(2,I)*D+B(2,J)*E
      C(3,K)=A(3,I)*D+B(3,J)*E
      RETURN
      END
      subroutine compact(WORD,L,K)
      implicit double precision (a-h,o-z)
      CHARACTER*(*) WORD
      K=0
      DO 1 I=1,L
      IF(WORD(I:I).LE.' '.OR.WORD(I:I).GT.'~')GO TO 1
      K=K+1
      WORD(K:K)=WORD(I:I)
    1 CONTINUE
      N=K+1
      DO 2 I=N,L
    2 WORD(I:I)=' '
      RETURN
      END
      subroutine compl_gr(MK,N,*)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CSM(NMG),CSMT,NMS,MTG(NMG,NMG)
      DIMENSION CO(3,3),MK(NMA,NMG)
complete the group
 2520 NN=NMS
      DO 2600 I=1,NN
      DO 2600 J=1,NN
      call prodmm(SIM,SIM,CO,I,J,1)
      DO 2560 JJ=1,NN
      L=ium(CO,SIM,1.d-2,1,JJ)
      IF(L.EQ.1)GO TO 2590
 2560 CONTINUE
      GO TO 2610
C group multiplication table
 2590 MTG(I,J)=JJ
 2600 CONTINUE
c Group completed
      RETURN 
C Has found a new matrix
 2610 NMS=NMS+1
      IF(NMS.LE.NMG) GO TO 2630
      write(7,2)
    2 format(' ERROR: TOO MANY MATRICES FOUND')
      write(7,'(3(3F10.5,/),/)')(((SIM(I,J,K),J=1,3),I=1,3),K=1,NMS)
c return by error
      RETURN 1
 2630 call trasfm(CO,SIM,1,NMS)
      do 2640 k=1,N
      k1=MK(k,J)
      MK(k,NMS)=MK(k1,I)
 2640 continue
      go to 2520
      end
      double precision function crms(t)
      implicit double precision (a-h,o-z)
C
C ppu=XO vector repeated NMS times
C ppo=vector XS transformed by MK (NMA, NMS)
C t(3)=vector angular variation in radians
C
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      PARAMETER (NMV=NMA*NMG)
      common/rmsmin/RV(3,3),ppo(3,NMV),ppu(3,NMV),npu
      dimension t(3),po(3),pu(3),rpu(3)
      dimension RX(3,3),RY(3,3),RZ(3,3),RR(3,3)
c     write(7,'(6F9.5)')t
      call reset(RX,0.d0,9)
      call reset(RY,0.d0,9)
      call reset(RZ,0.d0,9)
      call reset(RR,0.d0,9)
      RX(1,1)=1.d0
      RX(2,2)=dcos(t(1))
      RX(2,3)=-dsin(t(1))
      RX(3,2)=-RX(2,3)
      RX(3,3)=dcos(t(1))
      RY(2,2)=1.d0
      RY(1,1)=dcos(t(2))
      RY(1,3)=-dsin(t(2))
      RY(3,1)=-RY(1,3)
      RY(3,3)=dcos(t(2))
      RZ(1,1)=dcos(t(3))
      RZ(1,2)=-dsin(t(3))
      RZ(2,1)=-RZ(1,2)
      RZ(2,2)=dcos(t(3))
      RZ(3,3)=1.d0
c     write(7,'(3g12.5)')RX
c     write(7,*)
c     write(7,'(3g12.5)')RY
c     write(7,*)
c     write(7,'(3g12.5)')RZ
c     write(7,*)
      call prodmm(RY,RX,RR,1,1,1)
      call prodmm(RZ,RR,RV,1,1,1)
C precision orthonormalization
      call prodv(RV,RV,RV,1,2,3)
      call prodv(RV,RV,RV,3,1,2)
      call prodv(RV,RV,RV,2,3,1)
      call norm(RV,1)
      call norm(RV,2)
      call norm(RV,3)
c     write(7,'(3g12.5)')RV
C fine orthonormalization of precision
      func=0.d0
      do i=1,npu
       po(1)=ppo(1,i)
       po(2)=ppo(2,i)
       po(3)=ppo(3,i)
       pu(1)=ppu(1,i)
       pu(2)=ppu(2,i)
       pu(3)=ppu(3,i)
       call prodmv(RV,pu,pu,1,1,1)
c     write(7,'(6F9.5)')po,pu
       func=func+(po(1)-pu(1))**2+(po(2)-pu(2))**2+(po(3)-pu(3))**2
      enddo
      crms=func
c     write(7,*)func
      return
      end
      FUNCTION det(X,N)
      implicit double precision (a-h,o-z)
C DETERMINANT OF MATRIX X
      DIMENSION X(3,3,1)
      DET=  +X(1,1,N)*X(2,2,N)*X(3,3,N)-X(1,1,N)*X(2,3,N)*X(3,2,N)
      DET=DET+X(1,2,N)*X(2,3,N)*X(3,1,N)-X(1,2,N)*X(2,1,N)*X(3,3,N)
      DET=DET+X(1,3,N)*X(2,1,N)*X(3,2,N)-X(1,3,N)*X(2,2,N)*X(3,1,N)
      RETURN
      END
      subroutine eigen(A,VEC,EIG,W,GAM,BET,BSQ,P,Q,IPO,IORD,IVP,NN)
      implicit double precision (a-h,o-z)
C
C     --------------
C     QCPE VERSION
C     DECEMBER 1971
C     --------------
C
C     MATRIX DIAGNOLIZATION ROUTINE FOR REAL SYMMETRIC CASE
C     HOUSEHOLDER METHOD
C     RHO=UPPER LIMIT FOR OFF-DIAGONAL ELEMENT
C     NN SIZE OF MATRIX
C     A=MATRIX (ONLY LOWER TRIANGLE IS USED+THIS IS DESTROYED
C     EIG=RETURNED eigenVALUES IN ALGEBRAIC DESCENDING ORDER
C     VEC=RETURNED eigenVECTORS IN COLUMNS
C
C
      DIMENSION A(NN,NN),VEC(NN,NN),EIG(NN),W(NN),GAM(NN),BET(NN),
     *BSQ(NN),P(NN),Q(NN),IPO(NN),IORD(NN),IVP(NN)
C
      DATA RHOSQ/1.0D-12/
      ADUE=.50D0
      ZERO=0.D0
      UNO=1.0D0
      DUE=2.0D0
C
      N=NN
      IF(N)10,550,10
   10 N1=N-1
      N2=N-2
      GAM(1)=A(1,1)
      IF(N2) 180,170,20
   20 DO 160 NR=1,N2
      B=A(NR+1,NR)
      S=ZERO
      DO 30 I=NR,N2
   30 S=S+A(I+2,NR)**2
C     PREPARE FOR POSSIBLE BYPASS OF TRANSFORMATION
      A(NR+1,NR)=ZERO
      IF (S) 150,150,40
   40 S=S+B*B
      SGN=UNO
      IF (B) 50,60,60
   50 SGN=-UNO
   60 SQRTS=SQRT(S)
      D=SGN/(SQRTS+SQRTS)
      TEMP=SQRT(ADUE+B*D)
      W(NR)=TEMP
      A(NR+1,NR)=TEMP
      D=D/TEMP
      B=-SGN*SQRTS
C     D IS FACTOR OF PROPORTIONALITY. NOW COMPUTE AND SAVE W VECTOR.
C     EXTRA SINGLY SUBSCRIPTED W VECTOR USED FOR SPEED.
      DO 70 I=NR,N2
      TEMP=D*A(I+2,NR)
      W(I+1)=TEMP
   70 A(I+2,NR)=TEMP
C     PREMULTIPLY VECTOR W BY MATRIX A TO OBTAIN P VECTOR.
C     SIMULTANEOUSLY ACCUMULATE DOT PRODUCT WP,(THE SCALAR K)
      WTAW=ZERO
      DO 120 I=NR,N1
      I1=I+1
      SUM=ZERO
      DO 80 J=NR,I
   80 SUM=SUM+A(I1 ,J+1)*W(J)
      IF(N1-I1) 110,90,90
   90 DO 100 J=I1,N1
  100 SUM=SUM+A(J+1,I1 )*W(J)
  110 P(I)=SUM
      WWWI=W(I)
  120 WTAW=WTAW+SUM*WWWI
C     P VECTOR AND SCALAR K  NOW STORED. NEXT COMPUTE Q VECTOR
      DO 130 I=NR,N1
  130 Q(I)=P(I)-WTAW*W(I)
C     NOW FORM PAP MATRIX, REQUIRED PART
      DO 140 J=NR,N1
      QJ=Q(J)
      WJ=W(J)
      DO 140 I=J,N1
  140 A(I+1,J+1)=A(I+1,J+1)-DUE*(W(I)*QJ+WJ*Q(I))
  150 BET(NR)=B
      BSQ(NR)=B*B
  160 GAM(NR+1)=A(NR+1,NR+1)
  170 B=A(N,N1)
      BET(N1)=B
      BSQ(N1)=B*B
      GAM(N)=A(N,N)
  180 BSQ(N)=ZERO
C     ADJOIN AN IDENTIFY MATRIX TO BE POSTMULTIPLIED BY ROTATIONS.
      DO 200 I=1,N
      DO 190 J=1,N
  190 VEC(I,J)=ZERO
  200 VEC(I,I)=UNO
      M=N
      SUM=ZERO
      NPAS=1
      GO TO 330
  210 SUM=SUM+SHIFT
      COSA=UNO
      G=GAM(1)-SHIFT
      PP=G
      PPBS=PP*PP+BSQ(1)
      PPBR=SQRT(PPBS)
      DO 300 J=1,M
      COSAP=COSA
      IF(PPBS)230,220,230
  220 SINA=ZERO
      SINA2=ZERO
      COSA=UNO
      GO TO 270
  230 SINA=BET(J)/PPBR
      SINA2=BSQ(J)/PPBS
      COSA=PP/PPBR
C     POSTMULTIPLY IDENTITY BY P-TRANSPOSE MATRIX
      NT=J+NPAS
      IF(NT-N)250,240,240
  240 NT=N
  250 DO 260 I=1,NT
      TEMP=COSA*VEC(I,J)+SINA*VEC(I,J+1)
      VEC(I,J+1)=-SINA*VEC(I,J)+COSA*VEC(I,J+1)
  260 VEC(I,J)=TEMP
  270 DIA=GAM(J+1)-SHIFT
      U=SINA2*(G+DIA)
      GAM(J)=G+U
      G=DIA-U
      PP=DIA*COSA-SINA*COSAP*BET(J)
      IF(J-M)290,280,290
  280 BET(J)=SINA*PP
      BSQ(J)=SINA2*PP*PP
      GO TO 310
  290 PPBS=PP*PP+BSQ(J+1)
      PPBR=SQRT(PPBS)
      BET(J)=SINA*PPBR
  300 BSQ(J)=SINA2*PPBS
  310 GAM(M+1)=G
C     TEST FOR CONVERGENCE OF LAST DIAGONAL ELEMENT
      NPAS=NPAS+1
      IF(BSQ(M)-RHOSQ)320,320,350
  320 EIG(M+1)=GAM(M+1)+SUM
  330 BET(M)=ZERO
      BSQ(M)=ZERO
      M=M-1
      IF(M)340,380,340
  340 IF(BSQ(M)-RHOSQ)320,320,350
C     TAKE ROOT OF CORNER 2 BY 2 NEAREST TO LOWER DIAGONAL IN VALUE
C     AS ESTIMATE OF eigenVALUE TO USE FOR SHIFT
  350 A2=GAM(M+1)
      R2=ADUE*A2
      R1=ADUE*GAM(M)
      R12=R1+R2
      DIF=R1-R2
      TEMP=SQRT(DIF*DIF+BSQ(M))
      R1=R12+TEMP
      R2=R12-TEMP
      DIF=ABS(A2-R1)-ABS(A2-R2)
      IF(DIF)370,360,360
  360 SHIFT=R2
      GO TO 210
  370 SHIFT=R1
      GO TO 210
  380 EIG(1)=GAM(1)+SUM
C     INITIALIZE AUXILIARY TABLES REQUIRED FOR REARRANGING THE VECTORS
      DO 390 J=1,N
      IPO(J)=J
      IVP(J)=J
  390 IORD(J)=J
C     USE A TRANSPOSITION SORT TO ORDER THE eigenVALUES
      M=N
      GO TO 430
  400 DO 420 J=1,M
      IF(EIG(J)-EIG(J+1))410,420,420
  410 TEMP=EIG(J)
      EIG(J)=EIG(J+1)
      EIG(J+1)=TEMP
      ITEMP=IORD(J)
      IORD(J)=IORD(J+1)
      IORD(J+1)=ITEMP
  420 CONTINUE
  430 M=M-1
      IF(M)400,440,400
  440 IF(N1)450,490,450
  450 DO 480 L=1,N1
      NV=IORD(L)
      NP=IPO(NV)
      IF(NP-L)460,480,460
  460 LV=IVP(L)
      IVP(NP)=LV
      IPO(LV)=NP
      DO 470 I=1,N
      TEMP=VEC(I,L)
      VEC(I,L)=VEC(I,NP)
  470 VEC(I,NP)=TEMP
  480 CONTINUE
C     BACK TRANSFORM THE VECTORS OF THE TRIPLE DIAGONAL MATRIX
  490 DO 540 NRR=1,N
      K=N1
  500 K=K-1
      IF(K)540,540,510
  510 SUM=ZERO
      DO 520 I=K,N1
  520 SUM=SUM+VEC(I+1,NRR)*A(I+1,K)
      SUM=SUM+SUM
      DO 530 I=K,N1
  530 VEC(I+1,NRR)=VEC(I+1,NRR)-SUM*A(I+1,K)
      GO TO 500
  540 CONTINUE
  550 RETURN
      END
      subroutine eq_plane(t,u,v,a)
      implicit double precision (a-h,o-z)
      dimension t(3),u(3),v(3),AA(3,3),DD(3,3),a(4)
C   Equation of the plane passing through the points t, u, v in the canonical form
C   a(1).x+a(2).y+a(3).z+a(4)=0 
C   with a (1), a (2), a (3) = director cosines a(4)=-plane-origin distance
C see International Tables for Crystallography II, p. 43, equation 2,3,4, 6
      DD(1,1)=t(1)
      DD(1,2)=t(2)
      DD(1,3)=t(3)
      DD(2,1)=u(1)
      DD(2,2)=u(2)
      DD(2,3)=u(3)
      DD(3,1)=v(1)
      DD(3,2)=v(2)
      DD(3,3)=v(3)
      do i=1,3
      call trasfm(DD,AA,1,1)
      call reset(AA(1,i),1.d0,3)
      a(i)=det(AA,1)
      enddo
      call prods(a,a,dist,1,1,2)
      a(1)=a(1)/dist
      a(2)=a(2)/dist
      a(3)=a(3)/dist
      a(4)=-det(DD,1)/dist
      return
      end
      subroutine icosahed(XO,WEIGHT,N,MN,MK,MD,II,MDEG,*)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      CHARACTER*6 NAME
      COMMON/AT4/NAME(NMA)
      common/matcel/PC(7),PCR(7),O(3,3),OI(3,3),G(3,3),GI(3,3),CS(12)
      COMMON/AT0/NA,NMO
      COMMON/AT2/SX(3,NMA),SIG(NMA),DXM(NMA),DCM,DCME,indwgh,indtol
      COMMON/ORIE/OR(3,3),OT(3,3),OTI(3,3),BARC(3),BARO(3),RIN(3)
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CSM(NMG),CSMT,NMS,MTG(NMG,NMG)
      DIMENSION XO(3,NMA),WEIGHT(NMA),MD(NMA,2),MK(NMA,NMG),MN(NMA)
      DIMENSION DA(NMA),meq(NMA)
      dimension eqp(4),dp(5),vd(3,5),mp(5),io(5),A(3,3),B(3,3),V(3)
      logical ico
      RAD=57.29577951308232D0
      do 2000 I1=1,N-4
      if(MD(I1,1).ne.II)go to 2000
      mp(1)=I1
      io(1)=I1
      in1=I1+1
      do 1900 I2=in1,N-3
      if(MD(I1,1).ne.II)go to 1900
      mp(2)=I2
      in2=I2+1
      do 1800 I3=in2,N-2
      if(MD(I3,1).ne.II)go to 1800
      mp(3)=I3
      call eq_plane(XO(1,I1),XO(1,I2),XO(1,I3),eqp)
c     write(7,*)'equation of the plane'
c     write(7,'(4f10.6,3i5)')eqp,mp(1),mp(2),mp(3)
c     write(7,'(4f10.6,3i5)')DXM(I1),DXM(I2),DXM(I3),eqp(4)
C if the plane is too close to the origin it is discarded
      if(eqp(4).lt.0.5)go to 1800
      in3=I3+1
      do 1700 I4=in3,N-1
      if(MD(I4,1).ne.II)go to 1700
      mp(4)=I4
      d2=eqp(1)*XO(1,I4)+eqp(2)*XO(2,I4)+eqp(3)*XO(3,I4)+eqp(4)
      if(DABS(d2).gt.DXM(I4))go to 1700
c     write(7,'(A20,i5,f10.5)')'4th atom of the plane',mp(4),d2
 1300 in4=I4+1
      do 1600 I5=in4,N
      if(MD(I5,1).ne.II)go to 1600
      mp(5)=I5
      d2=eqp(1)*XO(1,I5)+eqp(2)*XO(2,I5)+eqp(3)*XO(3,I5)+eqp(4)
      if(DABS(d2).gt.DXM(I5))go to 1600
c     write(7,'(A20,i5,f10.5)')'5th atom of the plane',mp(5),d2
C in mp there are the possible 5 equivalent atoms with respect to axis 5
      dmin=100.
      dp(1)=dmin
      do i=2,5
        io(i)=0
        dp(i)=0.d0
        call combli(XO,XO,vd,1.d0,-1.d0,I1,mp(i),1)
        call prods(vd,vd,dp(i),1,1,2)
        if(dp(i).lt.dmin)then
          dmin=dp(i)
          jj=i
        endif
      enddo
      io(2)=mp(jj)
      dp(jj)=100.
      dmin=100.
      do i=2,5
        if(dp(i).lt.dmin.and.io(2).ne.mp(i))then
          dmin=dp(i)
          kk=i
        endif
      enddo 
      io(5)=mp(kk)
      do i=2,5
      if(mp(i).ne.io(2).and.mp(i).ne.io(5).and.io(3).eq.0)io(3)=mp(i)
      if(mp(i).ne.io(2).and.mp(i).ne.io(5).and.mp(i).ne.io(3))jj=mp(i)
      enddo
      call combli(XO,XO,vd,1.d0,-1.d0,io(2),io(3),1)
      call prods(vd,vd,d1,1,1,1)
      call combli(XO,XO,vd,1.d0,-1.d0,io(2),jj,1)
      call prods(vd,vd,d2,1,1,1)
      io(4)=jj
      if(d2.lt.d1)then
      io(4)=io(3)
      io(3)=jj
      endif
C now the five atoms are in order
C for preliminary check before going to verify, I check that the distances 1-2 are equal (less than the tolerance)
      dmed=0.
      do i=1,5
        k=i+1
        if(k.gt.5)k=k-5
        call combli(XO,XO,vd,1.d0,-1.d0,io(i),io(k),i)
        call prods(vd,vd,dp(i),i,i,2)
        dmed=dmed+dp(i)
      enddo
      dmed=dmed*0.2d0
c     write(7,'(a5,5i5)')'io',io
c     write(7,'(a5,5f10.5)')'dp',dp
c     write(7,'(a5,5f10.5)')'dmed',dmed
      call RESET(A,0.d0,9)
      do i=1,5
        if(DABS(dmed-dp(i)).gt.DXM(i)*.5)go to 1600
c axis C5 as the sum of the vertices of the pentagon
        A(1,3)=A(1,3)+XO(1,io(i))
        A(2,3)=A(2,3)+XO(2,io(i))
        A(3,3)=A(3,3)+XO(3,io(i))
      enddo
c     write(7,'(a5,3f10.6)')'A(3)',(A(i,3),i=1,3)
C sets the reference so that the first axis 5 coincides with z and that
C the second is in the xy plane
      A(1,1)=XO(1,io(1))+XO(1,io(2))-XO(1,io(3))-XO(1,io(4))+
     *XO(1,io(5))
      A(2,1)=XO(2,io(1))+XO(2,io(2))-XO(2,io(3))-XO(2,io(4))+
     *XO(2,io(5))
      A(3,1)=XO(3,io(1))+XO(3,io(2))-XO(3,io(3))-XO(3,io(4))+
     *XO(3,io(5))
C if the sum of the five atoms gives a null vector, that is the pentagon
C is on the great circle (possible case for a group of 30 atoms on
C the C2 axes) I pass to another pentagon
 1400 call norm(A,3)
      call norm(A,1)
      call prodv(A,A,A,3,1,2)
      call norm(A,2)
      call prodv(A,A,A,2,3,1)
      call traspo(A,A,1,1)
c     write(7,'(a5,3(/,3f10.6))')'A',((A(i,k),i=1,3),k=1,3)
      call r_frame(A,XO,N)
C Construction of the C5 axis
      call reset(B,0.d0,9)
      ROT=72.d0/RAD
      CA=COS(ROT)
      CB=SIN(ROT)
      B(1,1)=CA
      B(2,2)=CA
      B(1,2)=-CB
      B(2,1)=CB
      B(3,3)=1.d0
c     write(7,'(a5,3(/,3f10.6))')'B',((B(i,k),i=1,3),k=1,3)
c check and optimize the first axis 5
      call verify(XO,B,MK,MN,MV,N)
c     write(7,*)'primo asse C5 MV,NMS',MV,NMS
c     write(7,'(30i3)')(MK(ll,NMS),ll=1,N)
      if(MV.eq.0)go to 1600
      call opt_axis(XO,WEIGHT,V,MK,N,2)
c check and optimize the first axis 5
      call reset(A,0.d0,9)
      if(V(3).lt.0)then
      V(1)=-V(1)
      V(2)=-V(2)
      V(3)=-V(3)
      endif
      call trasfv(V,A,1,3)
      A(1,1)=XO(1,io(1))+XO(1,io(2))-XO(1,io(3))-XO(1,io(4))+
     *XO(1,io(5))
      A(2,1)=XO(2,io(1))+XO(2,io(2))-XO(2,io(3))-XO(2,io(4))+
     *XO(2,io(5))
      A(3,1)=XO(3,io(1))+XO(3,io(2))-XO(3,io(3))-XO(3,io(4))+
     *XO(3,io(5))
 1500 call norm(A,3)
      call norm(A,1)
      call prodv(A,A,A,3,1,2)
      call norm(A,2)
      call prodv(A,A,A,2,3,1)
      call traspo(A,A,1,1)
c     write(7,'(a5,3(/,3f10.6))')'A',((A(i,k),i=1,3),k=1,3)
      call r_frame(A,XO,N)
      go to 2010
 1600 continue
 1700 continue
 1800 continue
 1900 continue
 2000 continue
c normal output (found no C5 axis)
      if(NMS.eq.1)return
c completes group C5. Indispensable here!!!
 2010 call compl_gr(MK,N,*2015)
C I choose the two pentagons farthest from the C5 axis to find
C a C2 axis. in case the atoms of the subset are not
C all independent, there would be more than five atoms
C on the plane parallel to the first pentagon and therefore I must 
C test all possible pentagons
 2015 DM=0.d0
      do 2020 i=1,N
      if(MD(I1,1).ne.II)go to 2020
      DA(i)=XO(1,I)**2+XO(2,I)**2
      if(DM.gt.DA(i))go to 2020
      kk=i
      DM=DA(i)
 2020 continue
      io(1)=kk
      dp1=XO(3,kk)
c     write(7,*)'io(1),dp1',io(1),dp1
      do k=1,4
        k1=k+1
        io(k1)=MK(io(k),2)
        dp1=dp1+XO(3,io(k1))
      enddo
      dp1=-0.2d0*dp1
C in I (i) there are the five atoms of the first pentagon
C in meq (i) go the atoms of the parallel plane
      me=0
      do  2030 i=1,N
      com=DABS(dp1-XO(3,i))
      if(com.gt.DXM(i))go to 2030
C (eventual) elimination of the atoms of the first floor
      do k=1,5
        if(i.eq.io(k))go to 2030
      enddo
      me=me+1
      meq(me)=i
 2030 continue
c     write(7,*)'me,meq',me
c     write(7,'(30i4)')(meq(i),i=1,me)
C choice of the atoms of the second pentagon
 2040 do i=1,me
        if(meq(i).ne.0)go to 2050
      enddo
C there are no C2 axes!!!
      MDEG=1
      return
C i is the first atom of the new pentagon
 2050 mp(1)=meq(i)
      meq(i)=0
      k=1
C it looks for the other atoms of the second penate and cancels the relative meq
      do k=1,4
        k1=k+1
        mp(k1)=MK(mp(k),2)
        do l=1,me
          if(mp(k1).eq.meq(l))meq(l)=0
        enddo
      enddo
C sum vectors of the first pentagon suitably rotated
C construction of the C2 axis perpendicular to C5 and parallel to x
 2210 call reset(A,0.d0,9)
      call reset(vd,0.d0,15)
c     write(7,*)'io,mp'
c     write(7,'(5i4)')io,mp
      do 2250 i=1,5
      k=io(i)
      do 2220 l=1,NMS
      if(MK(k,l).ne.io(1))go to 2220
      call prodmv(SIM,XO,vd,l,k,3)
      call combli(vd,vd,vd,1.d0,1.d0,3,1,1)
c     write(7,'(a20,i5,3f10.5)')'io(i),vd(1)',io(i),(vd(kk,1),kk=1,3)
      go to 2230
 2220 continue
 2230 k=mp(i)
      do 2240 l=1,NMS
      if(MK(k,l).ne.mp(1))go to 2240
      call prodmv(SIM,XO,vd,l,k,3)
      call combli(vd,vd,vd,1.d0,1.d0,3,2,2)
c     write(7,'(a20,i5,3f10.5)')'mp(i),vd(2)',mp(i),(vd(kk,2),kk=1,3)
      go to 2250
 2240 continue
 2250 continue
      vd(3,1)=0.d0
      vd(3,2)=0.d0
      call norm(vd,1)
      call norm(vd,2)
c     write(7,'(a20,3f10.5)')'vd(1)',(vd(kk,1),kk=1,3)
c     write(7,'(a20,3f10.5)')'vd(2)',(vd(kk,2),kk=1,3)
      cost=DCOS(36.1/RAD)
      do i=1,5
        call prodmv(SIM,vd,vd,2,2,2)
        call prods(vd,vd,cosa,1,2,1)
c     write(7,'(a14,8f9.5)')'vd(1),vd(2)',(vd(kk,1),kk=1,3),
c    *(vd(kk,2),kk=1,3),cosa,cost
        if(cosa.ge.cost)go to 2300
      enddo
 2300 call combli(vd,vd,A,1.d0,1.d0,1,2,1)
      call norm(A,1)
c     write(7,'(a20,3f10.5)') 'A(1)',(A(kk,1),kk=1,3)
      A(3,3)=1.d0
      call prodv(A,A,A,3,1,2)
      call norm(A,2)
      call prodv(A,A,A,2,3,1)
      call norm(A,1)
      call traspo(A,A,1,1)
      call r_frame(A,XO,N)
c     write(7,'(i4,3f9.5)')(lll,(XO(kk,lll),kk=1,3),lll=1,N)
      call reset(B,0.d0,9)
      B(1,1)= 1.d0
      B(2,2)=-1.d0
      B(3,3)=-1.d0
      call verify(XO,B,MK,MN,MV,N)
c     write(7,*)'axis C2 MV,NMS',MV,NMS
      if(MV.eq.1)go to 2310
      call traspo(A,A,1,1)
      call r_frame(A,XO,N)
      call reset(A,0.d0,9)
      A(1,2)=-1.d0
      A(2,1)= 1.d0
      A(3,3)= 1.d0
      call r_frame(A,XO,N)
      call verify(XO,B,MK,MN,MV,N)
c     write(7,*)'axis C2 MV,NMS',MV,NMS
      if(MV.eq.1)go to 2310
      call traspo(A,A,1,1)
      call r_frame(A,XO,N)
      go to 2040
c     ico=.true.
c     call compl_gr(MK,N,*5000)
c     write(7,*)'asse C2,NMS',NMS
Check the center
 2310 call reset(B,0.d0,9)
      B(1,1)=-1.d0
      B(2,2)=-1.d0
      B(3,3)=-1.d0
      call verify(XO,B,MK,MN,MV,N)
c     write(7,*)'CENTRO MV,NMS',MV,NMS
Construction of the C5 axis at | x | = 63.43494882 from z and whose projection is at 18 from x
      call reset(A,0.d0,9)
      ROT=-63.43494882d0/RAD
 3000 CA=COS(ROT)
      CB=SIN(ROT)
      call reset(A,0.d0,9)
      A(1,1)=1.d0
      A(2,2)= CA
      A(3,3)= CA
      A(2,3)=-CB
      A(3,2)= CB
c rotation 18 on z
      call reset(B,0.d0,9)
      ROT18=-18.d0/RAD
 3100 CA=COS(ROT18)
      CB=SIN(ROT18)
      call reset(B,0.d0,9)
      B(1,1)= CA
      B(2,2)= CA
      B(1,2)=-CB
      B(2,1)= CB
      B(3,3)=1.d0
      call prodmm(B,A,A,1,1,1)
c     write(7,*)' rotation matrix'
c     write(7,'(a12,2f10.6)')'ROT,ROT18',ROT,ROT18
c     write(7,'(3f10.6)')((A(i,k),k=1,3),i=1,3)
      call prodmm(SIM,A,B,2,1,1)
      call traspo(A,A,1,1)
      call prodmm(A,B,A,1,1,1)
      call verify(XO,A,MK,MN,MV,N)
c     write(7,*)'second axis C5 ,MV,NMS',MV,NMS
      if(MV.eq.1)go to 4000
      if(ROT18.lt.0.d0)then
      ROT18=-ROT18
      go to 3100
      endif
      if(ROT.gt.0.d0)return
      ROT=-ROT
      go to 3000
 4000 call compl_gr(MK,N,*5000)
 5000 return 1
      END
      subroutine intpe(LINE,B,C,IA,NB,*,*)
      implicit double precision (a-h,o-z)
      CHARACTER*(*) LINE
      CHARACTER*23 CAR,CARA
C INTERPRETS THE VECTOR A AS AN EQUIVALENT POSITION OR
C PREPARE THE SAME FOR PRINTING STARTING FROM B AND C
      DIMENSION B(3,3,1),C(3,1)
      CAR='1234567890 /,+-xyz[]XYZ'
      GO TO (10,500),NB
C INTERPRETATION OF A
   10 MCM=1
      LR=LEN(LINE)
      call compact(LINE,LR,KM)
      K=1
      COST=1.d0
      DO 15 I=1,3
      C(I,IA)=0.d0
      DO 15 J=1,3
   15 B(I,J,IA)=0.d0
      DO 200 I=1,KM
C CHARACTER IDENTIFICATION
      CARA=LINE(I:I)
      DO 20 J=1,23
      IF(CAR(J:J).EQ.CARA)GO TO 30
   20 CONTINUE
   25 return 2
   30 IF(J-11)40,200,100
C NUMERICAL CHARACTER
   40 IF(J.EQ.10)J=0
      GO TO(50,60),MCM
C NUMERATOR
   50 IF(C(K,IA).NE.0.d0)C(K,IA)=C(K,IA)*10
      C(K,IA)=C(K,IA)+COST*J
      COST=1.d0
      GO TO 200
C DENOMINATOR
   60 C(K,IA)=C(K,IA)/J
      MCM=1
      GO TO 200
C ALPHABETICAL CHARACTERS
  100 if(J.gt.20)J=J-5
      J=J-11
      GO TO(110,120,130,140,150,150,150,200,210),J
C BAR
  110 MCM=2
      GO TO 200
C DIE LINE CHANGE (COMMA)
  120 K=K+1
      GO TO 200
C SIGN +
  130 COST=1.d0
      GO TO 200
C SIGN -
  140 COST=-1.d0
      GO TO 200
C  X,Y O Z
  150 J=J-4
      B(K,J,IA)=COST
      COST=1.d0
  200 CONTINUE
  210 RETURN 1
C VECTOR WRITING ALPHANUMERIC EQUIVALENT POSITION
  500 KM=LEN(LINE)
      DO 510 I=1,KM
  510 LINE(i:i)=car(11:11)
      LINE(12:12)=','
      LINE(23:23)=','
      DO 1000 I=1,3
      MCM=11*I
      DO 600 J=1,3
      L=4-J
      IF(NINT(B(I,L,IA)).EQ.0)GO TO 600
      MA=L+15
      LINE(MCM:MCM)=CAR(MA:MA)
      K=14
      IF(B(I,L,IA).LT.(-0.1d0))K=15
      MCM=MCM-1
      LINE(MCM:MCM)=CAR(K:K)
      MCM=MCM-1
  600 CONTINUE
      MS=11
      IF(ABS(C(I,IA)).LT.0.1d0)GO TO 1000
      IF(C(I,IA).LT.0.d0)MS=15
      DO 700 J=1,6
      CA=C(I,IA)*J
      K=NINT(CA)
      IF(ABS(FLOAT(K)-CA).LT.0.1d0)GO TO 800
  700 CONTINUE
  800 IF(J.EQ.1)GO TO 900
      LINE(MCM:MCM)=CAR(J:J)
      MCM=MCM-1
      LINE(MCM:MCM)=CAR(12:12)
      MCM=MCM-1
  900 J=IABS(MOD(K,10))
      LINE(MCM:MCM)=CAR(J:J)
      K=K/10
      MCM=MCM-1
      IF(K.GT.0)GO TO 900
      LINE(MCM:MCM)=CAR(MS:MS)
 1000 CONTINUE
      KM1=KM-1
      call compact(LINE(2:KM),KM1,K)
      if(LINE(2:2).eq.'+')LINE(2:2)=' '
      do 1010 i=1,2
      k=index(LINE,',+')
      k1=k+1
      if(k.ne.0)LINE(k1:k1)=' '
 1010 continue
      call compact(LINE(2:KM),KM1,K)
      RETURN 1
      END
      subroutine inv33(X,Y,K,L)
      implicit double precision (a-h,o-z)
      DIMENSION X(3,3,1),Y(3,3,1)
C INVERSION OF A MATRIX 3X3.
      C=1.0/det(X,K)
      Y(1,1,L)=(X(2,2,K)*X(3,3,K)-X(2,3,K)*X(3,2,K))*C
      Y(2,1,L)=(X(2,3,K)*X(3,1,K)-X(2,1,K)*X(3,3,K))*C
      Y(3,1,L)=(X(2,1,K)*X(3,2,K)-X(2,2,K)*X(3,1,K))*C
      Y(1,2,L)=(X(1,3,K)*X(3,2,K)-X(1,2,K)*X(3,3,K))*C
      Y(2,2,L)=(X(1,1,K)*X(3,3,K)-X(1,3,K)*X(3,1,K))*C
      Y(3,2,L)=(X(1,2,K)*X(3,1,K)-X(1,1,K)*X(3,2,K))*C
      Y(1,3,L)=(X(1,2,K)*X(2,3,K)-X(1,3,K)*X(2,2,K))*C
      Y(2,3,L)=(X(1,3,K)*X(2,1,K)-X(1,1,K)*X(2,3,K))*C
      Y(3,3,L)=(X(1,1,K)*X(2,2,K)-X(1,2,K)*X(2,1,K))*C
      RETURN
      END
      FUNCTION ium(A,B,C,M,N)
C CHECK IF A = ​​B LESS THAN C
C TRUE ium = 1   FALSE ium = 0
      implicit double precision (a-h,o-z)
      DIMENSION A(3,3,1),B(3,3,1)
      ium=0
      DO 1 I=1,3
      DO 1 J=1,3
      IF(ABS(A(I,J,M)-B(I,J,N)).GT.C)RETURN
    1 CONTINUE
      ium=1
      RETURN
      END
      subroutine capital(c)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      character c
      do 1 i=97,122
      if(c.ne.CHAR(i))go to 1
      c=CHAR(i-32)
      return
    1 continue
      return
      end
      subroutine mass(NAT,ATOM)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      CHARACTER*2 ATOM(NMA)
      CHARACTER*2 SYMBOL(103),ITT
      REAL*8 AMASS(103),RADIU(103)
      COMMON/AT1/X(3,NMA),AMAS(NMA),RCOV(NMA),MSP(NMA)
      DATA (AMASS(i),i=1,2)/1.00794,4.002602/
      DATA (AMASS(i),i=3,10)/6.941,9.012182,10.811,12.011,14.00674,
     115.9994,18.9984032,20.1797/
      DATA (AMASS(i),i=11,18)/22.989768,24.3050,26.981539,28.0855,
     130.97362,32.066,35.4527,39.948/
      DATA (AMASS(i),i=19,36)/39.0983,40.078,44.955910,47.88,
     1 50.9415,51.9961,54.93085,55.847,58.93320,58.69,63.546,65.39,
     2 69.723,72.61,74.92159,78.96,79.904,83.80/
      DATA (AMASS(i),i=37,54)/85.4678,87.62,88.90585,91.224,
     1 92.90638,95.94,98,101.07,102.90550,106.42,107.8682,112.411,
     2 114.82,118.710,121.75,127.60,126.90447,131.29/
      DATA (AMASS(i),i=55,86)/132.90543,137.327,138.9055,140.115,
     1 140.90765,144.24,145,150.36,151.965,157.25,158.92534,162.50,
     2 164.93032,167.26,168.93421,173.04,174.967,178.49,180.9479,
     3 183.85,186.207,190.2,192.22,195.08,196.96654,200.59,204.3833,
     4 207.2,208.98037,209,210.0,222/
      DATA (AMASS(i),i=87,103)/223,226.025,227.028,232.0381,
     1 231.03588,238.0289,237.048,244.,243.,247.,247.,251.,252.,
     2 257.,258.,259.,260.0/
      DATA (RADIU(i),i=1,2)/.32,.93/
      DATA (RADIU(i),i=3,10)/1.23,0.90,.82,.770,.75,.73,.72,.71/
      DATA (RADIU(i),i=11,18)/1.54,1.4,1.18,1.11,1.06,1.02,.99,.98/
      DATA (RADIU(i),i=19,36)/2.03,1.74,1.44,1.32,1.22,1.18,1.17,1.17,
     1 1.16,1.15,1.17,1.25,1.26,1.22,1.20,1.16,1.14,1.12/
      DATA (RADIU(i),i=37,54)/2.16,1.91,1.62,1.45,1.34,1.30,1.27,1.25,
     1 1.25,1.28,1.34,1.48,1.44,1.41,1.40,1.36,1.33,1.31/
C The radius of the elements Ce to Lu are interpolated
      DATA (RADIU(i),i=55,86)/2.35,1.98,1.69,1.67,1.65,1.63,1.61,1.59,
     1 1.57,1.55,1.53,1.51,1.49,1.47,1.45,1.45,1.45,1.44,1.34,1.30,1.28,
     2 1.26,1.27,1.30,1.34,1.49,1.48,1.47,1.46,1.76,1.45,1.45/
C The radius of the elements Ce to Lu are interpolated
      DATA (RADIU(i),i=87,103)/2.35,1.98,1.69,1.67,1.65,1.63,1.61,1.59,
     1 1.57,1.55,1.53,1.51,1.49,1.47,1.45,1.45,1.45/
      DATA (SYMBOL(i),i=1,2)/' H','HE'/
      DATA (SYMBOL(i),i=3,10)/'LI','BE',' B',' C',' N',' O',' F','NE'/
      DATA (SYMBOL(i),i=11,18)/'NA','MG','AL','SI',' P',' S','CL','AR'/
      DATA (SYMBOL(i),i=19,36)/' K','CA','SC','TI',' V','CR','MN','FE',
     1'CO','NI','CU','ZN','GA','GE','AS','SE','BR','KR'/
      DATA (SYMBOL(i),i=37,54)/'RB','SR',' Y','ZR','Nb','MO','TC','RU',
     1'RH','PD','AG','CD','IN','SN','SB','TE',' I','XE'/
      DATA (SYMBOL(i),i=55,86)/'CS','BA','LA','CE','PR','ND','PM',
     1'SM','EU','GD','TB','DY','HO','ER','TM','YB','LU','HF','TA',' W',
     2'RE','OS','IR','PT','AU','HG','TL','PB','BI','PO','AT','RN'/
      DATA (SYMBOL(i),i=87,103)/'FR','RA','AC','TH','PA',' U','NP',
     1'PU','AM','CM','BK','CF','ES','FM','MD','NO','LR'/
      DO J=1,NAT
        DO I=1,103
          IF(ATOM(J).EQ.SYMBOL(I)) THEN
            AMAS(J)=AMASS(I)
            RCOV(J)=RADIU(I)
c           write(7,'(a2,2f7.3)')ATOM(J),AMAS(J),RCOV(J)
            GOTO 2
          ENDIF
        ENDDO
        write(7,*)
        write(7,3) ATOM(J)
    3   format(' ERROR: MISSING DATA FOR THE ATOM-',A2,'-')
        STOP
    2   CONTINUE
      ENDDO
       DO J=1,NAT
       MSP(J)=0
       ENDDO
        M=1
        ITT=ATOM(1)
 10       DO 1 K=1,NAT
          IF(ATOM(K).EQ.ITT)MSP(K)=M
 1       CONTINUE
        M=M+1
        DO J=1,NAT
        IF(MSP(J).EQ.0)THEN
        ITT=ATOM(J)
        MSP(J)=M
        GOTO 10
        ENDIF
        ENDDO
      RETURN
        END
      subroutine momin(XO,P,RO,N)
      implicit double precision (a-h,o-z)
c from a set of XO coordinates that are multiplied by the RO matrix
C which can be an orthogonalisation matrix calculates the moments of inertia using P weights
      COMMON/ORIE/OR(3,3),OT(3,3),OTI(3,3),BARC(3),BARO(3),RIN(3)
      DIMENSION RO(3,3),AA(3,3),AMOM(6),CAM(3),ORI(3,3)
      DIMENSION C1(3),C2(3),C3(3),C4(3),C5(3),C6(3),IC1(3),IC2(3),IC3(3)
      DIMENSION XO(3,1),P(1)
      DO 100 I=1,3
      RIN(I)=0.d0
      AMOM(I)=0.d0
  100 AMOM(I+3)=0.d0
      SUM=0.d0
      DO 110 I=1,N
      call prodmv(RO,XO,XO,1,I,I)
      WEIGHT=P(I)
      RIN(1)=RIN(1)+WEIGHT*XO(1,I)
      RIN(2)=RIN(2)+WEIGHT*XO(2,I)
      RIN(3)=RIN(3)+WEIGHT*XO(3,I)
  110 SUM=SUM+WEIGHT
      DO 115 I=1,3
      RIN(I)=RIN(I)/SUM
  115 BARO(I)=RIN(I)
      DO 120 I=1,N
      WEIGHT=P(I)
      XO(1,I)=XO(1,I)-BARO(1)
      XO(2,I)=XO(2,I)-BARO(2)
      XO(3,I)=XO(3,I)-BARO(3)
      CAM(1)=XO(1,I)*WEIGHT
      CAM(2)=XO(2,I)*WEIGHT
      CAM(3)=XO(3,I)*WEIGHT
      AMOM(1)=AMOM(1)+CAM(1)*XO(1,I)
      AMOM(2)=AMOM(2)-CAM(1)*XO(2,I)
      AMOM(3)=AMOM(3)-CAM(1)*XO(3,I)
      AMOM(4)=AMOM(4)+CAM(2)*XO(2,I)
      AMOM(5)=AMOM(5)-CAM(2)*XO(3,I)
      AMOM(6)=AMOM(6)+CAM(3)*XO(3,I)
  120 CONTINUE
      AA(1,1)=AMOM(4)+AMOM(6)
      AA(2,2)=AMOM(1)+AMOM(6)
      AA(3,3)=AMOM(1)+AMOM(4)
      AA(1,2)=AMOM(2)
      AA(2,1)=AMOM(2)
      AA(1,3)=AMOM(3)
      AA(3,1)=AMOM(3)
      AA(2,3)=AMOM(5)
      AA(3,2)=AMOM(5)
      call eigen(AA,OR,RIN,C1,C2,C3,C4,C5,C6,IC1,IC2,IC3,3)
C__________________
C modification to make OR machine-independent
      am1=-1
      am2=-1
      do i=1,3
        if(DABS(OR(i,1)).gt.am1)then
          am1=DABS(OR(i,1))
          n1=i
        endif
        if(DABS(OR(i,2)).gt.am2)then
          am2=DABS(OR(i,2))
          n2=i
        endif
      enddo
      if(OR(n1,1).lt.0.0)then
        do i=1,3
          OR(i,1)=-OR(i,1)
        enddo
      endif
      if(OR(n2,1).lt.0.0)then
        do i=1,3
          OR(i,2)=-OR(i,2)
        enddo
      endif
      CALL prodv(OR,OR,OR,1,2,3)
C__________________
      call traspo(OR,OR,1,1)
      call prodmm(OR,RO,OT,1,1,1)
      call inv33(OT,OTI,1,1)
      call inv33(RO,ORI,1,1)
      call prodmv(ORI,BARO,BARC,1,1,1)
      do 130 i=1,N
  130 call prodmv(OR,XO,XO,1,I,I)
      RETURN
      END
      subroutine norm(A,I)
      implicit double precision (a-h,o-z)
C VECTOR A NORMALIZATION
      DIMENSION A(3,1)
      call prods(A,A,B,I,I,2)
      DO 1 J=1,3
    1 A(J,I)=A(J,I)/B
      RETURN
      END
      subroutine opt_axis(XO,WEIGHT,V3,MK,N,IAX)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CSM(NMG),CSMT,NMS,MTG(NMG,NMG)
      DIMENSION XO(3,NMA),MK(NMA,NMG),V3(3),WEIGHT(NMA),VS(3)
      DIMENSION IV(NMA),CO(3,NMA),W(NMA),nge(NMA)
C causes the sum of the distances to the IAX symmetry element be minimal
      do i=1,N
        iv(i)=0
      enddo
      ng=0
      cost=det(SIM,IAX)
 1000 do I=1,N
         if(iv(i).eq.0)go to 1100
      enddo
      go to 1300
 1100 ng=ng+1
C ng = number of the group of atoms equivalent to the IAX operation
      iv(i)=ng
      do j=1,3
         CO(j,ng)=XO(j,I)
      enddo
      W(ng)=WEIGHT(I)
      nge(ng)=1
      k=I
 1200 l=MK(k,IAX)
      if(l.eq.I)go to 1000
      iv(l)=ng
C sum all the atoms of the group with the appropriate sign
      do j=1,3
         CO(j,ng)=CO(j,ng)+cost*XO(j,l)
      enddo
      W(ng)=W(ng)+WEIGHT(l)
      nge(ng)=nge(ng)+1
      k=l
      go to 1200
C assigned all the atoms to a group and the weight to the groups
 1300 continue    
      do i=1,ng
      W(I)=W(i)/nge(I)
      enddo
      call reset(V3,0.d0,3)
      dism=0.d0
c Search for the group furthest from the symmetry element (im)
      do i=1,ng 
         call prodmv(SIM,CO,VS,IAX,i,1)
         call combli(CO,VS,VS,1.d0,cost,i,1,1)
         call prods(VS,VS,dis,1,1,1)
c        write(7,'(i3,6f10.5)')i,(CO(j,i),j=1,3),VS
         if(dis.gt.dism)then
            dism=dis
            im=i
         endif
      enddo
      call reset(V3,0.d0,3)
         call prods(CO,CO,dim,im,im,2)
c      write(7,'(6him,dim,i3,3f10.5)')im,dim
c adds together all the groups with a sign determined by the angle between the group in question and the im group
      do i=1,ng
         call prods(CO,CO,abcos,im,i,1)
         call prods(CO,CO,di,i,i,2)
         absabcos=abs(abcos)
         if(absabcos.gt.1.d-5)then
            cost=W(I)*abcos/absabcos
            call combli(V3,CO,V3,1.d0,cost,1,i,1)
         endif
      enddo
c the resulting vector is normalized and will become a line of the definitive rotation matrix
      call norm(V3,1)
      if(V3(1).gt.0.d0)return
      V3(1)=-V3(1)
      V3(2)=-V3(2)
      V3(3)=-V3(3)
      return
      end
      subroutine out_bond(XO,IEQAT,IASU,N)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      CHARACTER*6 NAME
      COMMON/AT4/NAME(NMA)
      COMMON/AT5/BONDDD(20,NMA),NLEG(NMA)
      DIMENSION T(3),V(3),IASU(NMA),tableg(20,NMA),XO(3,NMA),IEQAT(NMA)
      CHARACTER*80 line
      write(7,*)
c     write(7,'(a6,3f10.6)')(NAME(IEQAT(I)),(XO(k,i),k=1,3),i=1,N)
      RAD=57.29577951308232D0
      line=' '
      nb=0
      do 1400 i=1,N
      if(IASU(i).ne.2)go to 1400
      N1=NLEG(i)
      do 1300 k=1,N1
      l=BONDDD(k,i)
      call combli(XO,XO,T,1.d0,-1.d0,i,l,1)
      call prods(t,t,tableg(k,i),1,1,2)
      nc=nb*26+1
      nb=nb+1
      write(line(nc:nc+25),'(a6,a1,a6,f7.4,6x)')NAME(IEQAT(I)),'-',
     *NAME(IEQAT(l)),tableg(k,i)
      if(nb.eq.3)then
        write(7,'(a)')line
        nb=0
        line=' '
      endif
 1300 continue
 1400 continue
      if(nb.ne.0)write(7,'(a)')line
      write(7,*)
        line=' '
        nb=0
      do 2000 i=1,N
      N1=NLEG(i)
      if(IASU(i).ne.2)go to 2000
      do 1900 k=1,N1-1
      k1=k+1
      i1=BONDDD(k,i)
      call combli(XO,XO,T,1.d0,-1.d0,i1,i,1)
      do 1800 l=k1,N1
      i2=BONDDD(l,i)
      call combli(XO,XO,V,1.d0,-1.d0,i2,i,1)
      call prods(T,V,cost,1,1,1)
      cost=cost/(tableg(k,i)*tableg(l,i))
      CALL arccos(cost,ang)
      nc=nb*35+1
      nb=nb+1
      write(line(nc:nc+27),'(a6,2(a1,a6),f8.3,7x)')NAME(IEQAT(I1)),
     *'-',NAME(IEQAT(I)),'-',NAME(IEQAT(I2)),ang
      if(nb.eq.2)then
        write(7,'(a)')line
        nb=0
        line=' '
      endif
 1800 continue
 1900 continue
 2000 continue
      if(nb.ne.0)write(7,'(a)')line
      write(7,*)
      return
      END
      subroutine pratba(B,A,C,I,J,K)
      implicit double precision (a-h,o-z)
C                    T
C TRANSFORMATION  C=A BA
      DIMENSION A(3,3,1),B(3,3,1),C(3,3,1),D(3,3)
      call PRODMM(B,A,C,I,J,K)
      call TRASPO(A,D,J,1)
      call PRODMM(D,C,C,1,K,K)
      RETURN
      END
      subroutine prodmm(A,B,C,I,J,N)
      implicit double precision (a-h,o-z)
      DIMENSION A(3,3,1),B(3,3,1),C(3,3,1),X(3,3)
      DO 1 K=1,3
      DO 1 L=1,3
      X(K,L)=0.0d0
      DO 1 M=1,3
      Y=A(K,M,I)
      Z=B(M,L,J)
    1 X(K,L)=X(K,L)+Y*Z
      DO 2 K=1,3
      DO 2 L=1,3
    2 C(K,L,N)=X(K,L)
      RETURN
      END
      subroutine prodms(A,B,C,M,N)
      implicit double precision (a-h,o-z)
      DIMENSION A(3,3,1), B(3,3,1)
      X=C
      DO 1 I=1,3
      DO 1 J=1,3
      Y=A(I,J,M)
    1 B(I,J,N)=X*Y
      RETURN
      END
      subroutine prodmv(A,B,C,I,J,K)
      implicit double precision (a-h,o-z)
      DIMENSION A(3,3,1),B(3,1),C(3,1),X(3)
      DO 1 L=1,3
      X(L)=0.0d0
      DO 1 M=1,3
      Y=A(L,M,I)
      Z=B(M,J)
    1 X(L)=X(L)+Y*Z
      DO 2 L=1,3
    2 C(L,K)=X(L)
      RETURN
      END
      subroutine prods(A,B,C,I,J,K)
      implicit double precision (a-h,o-z)
C SCALAR PRODUCT A*B=C (K=1)
C  MODULO VECTOR A        (K=2)
      DIMENSION A(3,1),B(3,1)
      X=0.0d0
      DO 1 L=1,3
      Y=A(L,I)
      Z=B(L,J)
    1 X=X+Y*Z
      C=X
      IF(K.EQ.2)C=DSQRT(X)
      RETURN
      END
      subroutine prodv(A,B,C,I,J,K)
      implicit double precision (a-h,o-z)
C VECTORIAL PRODUCT A*B=C
      DIMENSION A(3,1),B(3,1),C(3,1)
      X1=A(1,I)
      X2=A(2,I)
      X3=A(3,I)
      Y1=B(1,J)
      Y2=B(2,J)
      Y3=B(3,J)
      C(1,K)=X2*Y3-X3*Y2
      C(2,K)=-X1*Y3+X3*Y1
      C(3,K)=X1*Y2-X2*Y1
      RETURN
      END
      subroutine r_frame(CO,XO,N)
C rotate the reference and coordinates
      implicit double precision (a-h,o-z)
      COMMON/ORIE/OR(3,3),OT(3,3),OTI(3,3),BARC(3),BARO(3),RIN(3)
      DIMENSION CO(3,3),XO(3,*)
c     write(7,*)'rotation of the reference'
c     write(7,'(3f10.6)')((CO(i,k),k=1,3),i=1,3)
      DO 1010 I=1,N
 1010 call prodmv(CO,XO,XO,1,I,I)
      call prodmm(CO,OR,OR,1,1,1)
      call prodmm(CO,OT,OT,1,1,1)
      call inv33(OT,OTI,1,1)
c     write(7,*)'new coordinates'
c     write(7,'(i5,3f10.6)')(i,(XO(k,i),k=1,3),i=1,N)
      return 
      end
      subroutine rms_min(tm)
      implicit double precision (a-h,o-z)
C
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      PARAMETER (NMV=NMA*NMG)
      common/rmsmin/RV(3,3),ppo(3,NMV),ppu(3,NMV),npu
      dimension tm(3),t(3)
      passo=0.02
      call reset(RV,0.d0,9)
      call reset(tm,0.d0,3)
      rm=crms(tm)
    1 ind1=0
      do i1=1,3
       t(1)=tm(1)+(2-i1)*passo
       do i2=1,3
        t(2)=tm(2)+(2-i2)*passo
        do i3=1,3
         t(3)=tm(3)+(2-i3)*passo
         arms=crms(t)
C        write(7,*)t,arm,i1,i2,i3
         if(arms.lt.rm)then
          ind1=1
          rm=arms
c         write(7,*)'rm,passo',rm,passo
          call trasfv(t,tm,1,1)
         endif
        enddo
       enddo
      enddo
      if(ind1.eq.1)goto 1
      if(passo.lt.0.0002)return
      passo=passo*0.5
      go to 1
      end
      subroutine s_coor(SI,XO,XS,CO,D,DEL,DLM,CSM,MK,ID,ISU,nes,N)
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      COMMON/SIMME1/RMS(3,NMA),RMST
      DIMENSION CO(3,NMA),XO(3,NMA),XS(3,NMA),SI(3,3,NMG),D(NMA)
      DIMENSION DEL(4),DLM(4),V3(3),ID(4),MK(NMA,NMG),ISU(NMG)
      DIMENSION dc(NMA),cf(NMA)
calculates the symmetrized coordinates XS
c     write(7,'(i3,3f10.5,)')(kk,(XO(ll,kk),ll=1,3),kk=1,N)
      NAZ=3*N
      call reset(CO,0.d0,NAZ)
      call reset(XS,0.d0,NAZ)
      call reset(RMS,0.d0,NAZ)
      call reset(DEL,0.d0,4)
      call reset(DLM,0.d0,4)
      call reset(cf,0.d0,N)
C ISU = index of the matrices of the subgroup under consideration
C nes = number of elements of the subgroup
c     write(7,'(a3,12i3)')'ISU',(ISU(J),J=1,nes)
      DO 2800 I=1,N
      call prods(XO,XO,D(I),I,I,2)
      DO 2790 J=1,nes
       K=MK(I,ISU(j))
       call prodmv(SI,XO,V3,ISU(J),I,1)
       call combli(XS,V3,XS,1.d0,1.d0,K,1,K)
 2790 continue
c     write(7,'(i2,4f9.5)')I,(XO(kk,I),kk=1,3),D(I)
 2800 CONTINUE
      do i=1,N
       do J=1,3
        XS(J,I)=XS(J,I)/dfloat(nes)
       enddo
       call prods(XS,XS,dc(i),i,i,2)
      enddo
      do I=1,N
        DO J=1,nes
          K=MK(I,ISU(j))
          cf(I)=cf(I)+D(K)
        enddo
        if(dc(i).gt.1.d-01)cf(i)=cf(i)/(dc(i)*nes)
        do J=1,3
          XS(J,I)=XS(J,I)*cf(i)
        enddo
      enddo
c     write(7,'(i2,6f10.6)')1,(XS(kk,1),kk=1,3),dc(1),cf(1)
C the correction factor cf = average of the distances atoms equivalent to the center of gravity,
C corrects the symmetrized coordinates making them equal to
C mean by pure rotation of the equivalent atoms
C calculates the rms and the average (DEL) and maximum DLM deviations
      do j=1,4
       ID(j)=0
      enddo
      DO 2820 i=1,N
       do j=1,3
        V3(j)=XO(j,i)-XS(j,i)
        com=dabs(V3(j))
        DEL(j)=DEL(J)+com
        if(com.gt.DLM(j))then
          DLM(j)=com
          ID(j)=I
        endif
       enddo
C maximum deviation from symmetry in angstroms
      call prods(V3,V3,com,1,1,2)
      DEL(4)=DEL(4)+com
      if(DLM(4).lt.com)then
       DLM(4)=com
       ID(4)=I
      endif
 2820 continue
C calculate continuous symmetry measure (CSM)
      CSM=0.d0
      do j=1,nes
       do i=1,N
        K=MK(i,ISU(j))
        call prodmv(SI,XO,V3,ISU(J),i,1)
        call combli(XS,V3,V3,1.d0,-1.d0,K,1,1)
        RMS(1,K)=RMS(1,K)+V3(1)*V3(1)
        RMS(2,K)=RMS(2,K)+V3(2)*V3(2)
        RMS(3,K)=RMS(3,K)+V3(3)*V3(3)
        call prods(V3,V3,com,1,1,1)
        CSM=CSM+com
       enddo
      enddo
      cost=1.d0/dfloat(nes)
      RMST=0.d0
      do I=1,N
       do j=1,3
       RMST=RMST+RMS(j,i)
       RMS(j,i)=sqrt(RMS(j,i)*cost)
       enddo
      enddo
      RMST=sqrt(RMST/dfloat(nes*N))
      CSM=CSM*1.d2/dfloat(nes*N)
      return
      end
      subroutine schoenfl(MDEG)
C From the group matrices gives the Schoenflies symbol
      implicit double precision (a-h,o-z)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CMS(NMG),CSMT,NMS,MTG(NMG,NMG)
      COMMON/SIMME1/RMS(3,NMA),RMST
      character*3 lbls,pt*7
      character S1,S2,Sn*2,tot*4
      COMMON/SIMME3/IPOGROU,lbls(NMG)
      character*5 pointgr(32)
      DATA (pointgr(i),i=1,32)
     */'C1 ','Ci ','C2 ','Cs ','C2h','D2 ','C2v','D2h','C4 ','S4 ','C4h'
     *,'D4 ','C4v','D2d','D4h','C3 ','C3i','D3 ','C3v','D3d','C6 ','C3h'
     *,'C6h','D6 ','C6v','D3h','D6h','T  ','Th ','O  ','Td ','Oh '/
      IPOGROU=0
      maxasp=0
      maxasi=0
      mplane=0
      invers=0
      nplane=0
      lbls(1)='E'
C_______________________________________________________
C linear groups
C for these groups I do not print the symmetry matrices but only the group
C
      if(MDEG.eq.5)then
        pt='C(inf)v'
        do i=1,NMS
          call ax_order(SIM,i,m,msign,inv)
          if(inv.eq.1)go to 1000
        enddo
        NMS=1
        go to 5200
 1000   pt='D(inf)h'
        NMS=1
        go to 5200
      endif
C_______________________________________________________
      if(NMS.eq.1)then
        pt='C1 '
        IPOGROU=1
        return
      endif
      do i=2,NMS
        tot='    '
        S1=' '
        S2=' '
        Sn='  '
        call ax_order(SIM,i,m,msign,inv)
        if(m.gt.maxasp.and.msign.eq.1)maxasp=m
        if(m.gt.maxasi.and.msign.eq.-1)maxasi=m
        if(m.eq.2.and.mplane.eq.0)mplane=1
c     write(7,'(a20,4i3)')'i,msign,m,inv',i,msign,m,inv
        S1='C'
        if(m.eq.2)then
C center
          if(inv.eq.1)then
            invers=1
            Sn='i '
            go to 1100
          endif
C plane
          Sn='2 '
C axis 2
          if(msign.eq.-1)then
            Sn='s '
            nplane=nplane+1
          endif
          go to 1100
        endif
C improper axis m>2
        write(Sn,'(i2)')m
        if(msign.eq.-1)S1='S'
 1100   tot=S1//Sn//S2
        call compact(tot,4,k)
        lbls(i)=tot(1:3)
      enddo
c     write(7,*)'maxasp maxasi mplane nplane invers   MDEG    NMS'
c     write(7,'(7i7)')maxasp,maxasi,mplane,nplane,invers,MDEG,NMS
      pt='   '
      if(NMS.eq.48)pt='Oh '
      if(NMS.eq.60)pt='I  '
      if(NMS.eq.120)pt='Ih '
      if(pt.ne.'   ')go to 5100
      if(MDEG.eq.3)then
        if(NMS.eq.12)pt='T23'
        if(invers.eq.1)pt='Th '
        if(maxasi.eq.4)pt='Td '
        if(pt.eq.'   ')pt='O  '
        go to 5100
      endif
      if(NMS.eq.2)then
        pt=lbls(2)
        go to 5000
      endif
      S2=' '
      S1='C'
C Cn and S2n where n=maxasp
      write(Sn,'(I2)')maxasp
      if(MOD(NMS,2).eq.0.and.maxasi.eq.NMS)then
        S1='S'
        write(Sn,'(I2)')maxasi
        if(NMS.ne.6)go to 5000
        pt='C3i'
        go to 5100
      endif
      if(NMS.eq.maxasp)go to 5000
      if(NMS.lt.maxasi)then
        write(Sn,'(I2)')maxasi
        S1='S'
        go to 5000
      endif
C Dn, Cnv e Cnh
      S1='D'
      if(NMS.eq.maxasp*2)then
        if(nplane.eq.0)go to 5000
        S1='C'
        S2='v'
        if(nplane.eq.maxasp)go to 5000
        S2='h'
        go to 5000
      endif
      S2='h'
      if(nplane.eq.maxasp)S2='d'
    1 format(/,' Schoenflies symbol = ',a7,'  CSM =',f8.4,5x,
     *'Molecular RMS =',f8.4)
    2 format(' CSM,see: Zabrodsky et al. (1993) JACS, 115, 8278-8298')
 5000 tot='    '
      tot=S1//Sn//S2
      call compact(tot,4,k)
      pt=tot(1:3)
 5100 do m=1,32
        if(pt.eq.pointgr(m))IPOGROU=m
      enddo
 5200 write(7,1)pt,CSMT,RMST
      write(7,2)
      RETURN
      END
      subroutine stasy
      implicit double precision (a-h,o-z)
C
C PRINT THE EQUIVALENT POSITIONS
C
c     character*120 linea
      PARAMETER (NMG=120)
      character*80 linea
      character*40 line
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CSM(NMG),CSMT,NMS,MTG(NMG,NMG)
      dimension TRL(3,NMG)
      character*3 lbls
      COMMON/SIMME3/IPOGROU,lbls(NMG)
      call reset(TRL,0.d0,3*NMS)
      i1=-39
      I=1
      if(NMS.gt.1)I=2
    2 format(2a40)
      write(7,2)(' Symmetry element its CSM and Max.Diff. ',k=1,I)
      DO 320 M=1,NMS
      mm=M
      call intpe(line(1:39),SIM,TRL,mm,2,*310,*310)
  310 i1=i1+40
      i2=i1+39
      i3=I1+21
      i4=I1+37
      write(linea(i1:i2),3)M,lbls(M),line
    3 format(I2,') [',a3,'] ',a30)
      write(linea(i3:i4),'(2f8.4)')CSM(M),DEV(M)
c     if(i1.eq.81)then
      if(i1.eq.41)then
          write(7,'(1x,a)')linea
      do 315 kk=1,80
c     do 315 kk=1,120
      linea(kk:kk)=' '
  315 continue
          i1=-39
      endif
  320 CONTINUE
      if(i1.ge.1)write(7,'(1x,a)')linea(1:i1+39)
      RETURN
      END
      subroutine trasfm(A,B,I,J)
      implicit double precision (a-h,o-z)
C TRANSFER OF A MATRIX
      DIMENSION A(3,3,1),B(3,3,1)
      DO 1 K=1,3
      DO 1 L=1,3
    1 B(K,L,J)=A(K,L,I)
      RETURN
      END
      subroutine trasfv(A,B,I,J)
      implicit double precision (a-h,o-z)
C TRANSFER OF A VECTOR
      DIMENSION A(3,1),B(3,1)
      B(1,J)=A(1,I)
      B(2,J)=A(2,I)
      B(3,J)=A(3,I)
      RETURN
      END
      subroutine traspo(A,B,M,N)
      implicit double precision (a-h,o-z)
      DIMENSION A(3,3,1),B(3,3,1),C(3,3)
      DO 1 I=1,3
      DO 1 J=1,3
    1 C(I,J)=A(J,I,M)
      call trasfm(C,B,1,N)
      RETURN
      END
      subroutine verify(XO,A,MK,MN,MV,N)
      implicit double precision (a-h,o-z)
C VERIFY IF A IS A SYMMETRY MATRIX LESS THAN DXM
C IF SO IT SETS MV = 1, NMS = NMS + 1, SIM (I, J, NMS = A (I, J)
C MK (I, NMS) CONTAINS THE NUMBER OF THE ATOM OBTAINED BY STASFORMING I WITH THE NMS OPERATION
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CSM(NMG),CSMT,NMS,MTG(NMG,NMG)
      COMMON/AT2/SX(3,NMA),SIG(NMA),DXM(NMA),DCM,DCME,indwgh,indtol
      DIMENSION XO(3,1),MN(1),MK(NMA,1),A(3,3),V(3),W(3),iat(NMA)
      MV=1
C First check that matrix A is not already included in the SIM
      do i=1,NMS
        if(ium(A,SIM,1.d-2,1,i).eq.1)go to 6
      enddo
      NMS=NMS+1
c     write(7,'(i3,3f10.6,i4)')(i,(XO(k,i),k=1,3),MN(i),i=1,N)
c     write(7,'(3f10.6)')((A(k,i),i=1,3),k=1,3)
      DO 4 I=1,N
      MK(I,NMS)=0
      JJ=0
      FMIN=DXM(I)
      call prodmv(A,XO,V,1,I,1)
c     write(7,'(3f10.6)')V
      DO 2 J=1,N
      IF(MN(I).NE.MN(J))GO TO 2
      call combli(XO,V,W,1.d0,-1.d0,J,1,1)
      call prods(W,W,F,1,1,2)
c     write(7,*)'I,J,NMS,F',I,J,NMS,F
      IF(F.GT.FMIN)GO TO 2
      FMIN=F
      JJ=J
    2 CONTINUE
c     write(7,*)'JJ, if JJ=0 refuses',JJ
      IF(JJ.EQ.0)GO TO 5
      MK(I,NMS)=JJ
    4 CONTINUE
c     write(7,*)'MK'
c     write(7,'(29i3)')(MK(k,NMS),k=1,N)
C analysis of the multiplication matrix
      do k=1,N
      iat(k)=0
      enddo
      do 10 k=1,N
      if(iat(k).ne.0)go to 10
      iat(k)=1
      neq=1
      l=k
    7 m=MK(l,NMS)
      if(m.eq.k)go to 9
      iat(m)=1
      neq=neq+1
      if(neq.gt.N)go to 5
      l=m
      go to 7
    9 continue
      call ax_order(A,1,nord,msign,inv)
c     write(7,*)'nord,neq,inv',nord,neq,inv
C the number of equivalent atoms for the operation of order m can be
C only 1 or 2 (atoms lying on the symmetry element) m (normal case)
C o 2m (when you have a reducible element 3 / m 5 / m etc.)
      if(neq.eq.1.or.neq.eq.2)go to 10
      if(neq.eq.nord)go to 10
      if(neq.eq.2*nord)go to 10
      go to 5
   10 continue
c     write(7,*)'accepted, nord=',nord
c     write(7,'(3f10.5)')((A(i,j),j=1,3),i=1,3)
      call trasfm(A,SIM,1,NMS)
      RETURN
    5 NMS=NMS-1
    6 MV=0
c     write(7,*)'return from verify,NMS=',NMS
      RETURN
      END
      subroutine work(MO)
C MO      = molecule to be symmetrized
C X       = original fractional coordinates
C NA      = total number of atoms
C MOL(I)  = molecule to which atom I belongs
C MSP(I)  = indicator of the species of atom I
C AMAS(I) = mass of atom I
C XO      = coordinates only atoms to be symmetrized
C N       = number of atoms to be symmetrized
C SIM     = symmetry matrices of the molecular group
C NMS     = order of the symmetry group found
      implicit double precision (a-h,o-z)
      PARAMETER (maxorder=8)
      PARAMETER (NMA=1000)
      PARAMETER (NMG=120)
      PARAMETER (NMV=NMA*NMG)
      common/rmsmin/RV(3,3),ppo(3,NMV),ppu(3,NMV),npu
      CHARACTER*6 NAME,NNAME
      CHARACTER*52 comment
      COMMON/AT4/NAME(NMA)
      common/matcel/PC(7),PCR(7),O(3,3),OI(3,3),G(3,3),GI(3,3),CS(12)
      COMMON/AT0/NA,NMO
      COMMON/AT1/X(3,NMA),AMAS(NMA),RCOV(NMA),MSP(NMA)
      COMMON/AT2/SX(3,NMA),SIG(NMA),DXM(NMA),DCM,DCME,indwgh,indtol
      COMMON/AT3/MOL(NMA),MLG(NMA)
      COMMON/ORIE/OR(3,3),OT(3,3),OTI(3,3),BARC(3),BARO(3),RIN(3)
      COMMON/SIMME/SIM(3,3,NMG),DEV(NMG),CSM(NMG),CSMT,NMS,MTG(NMG,NMG)
      COMMON/SIMME1/RMS(3,NMA),RMST
      COMMON/SIMME2/AL(3,3),ALX(3,3)
      CHARACTER*3 lbls
      COMMON/SIMME3/IPOGROU,lbls(NMG)
      DIMENSION MK(NMA,NMG),MN(NMA),XO(3,NMA),XS(3,NMA),MD(NMA,2)
      DIMENSION D(NMA),AA(3,3),CO(3,NMA),IEQAT(NMA)
      DIMENSION WEIGHT(NMA),DEL(4),DELM(4),IDM(4),delta(3,NMA)
      DIMENSION V(3),W(3),NNAME(4),ISU(NMG),IASU(NMA)
      CHARACTER type(2),car
      type(1)=' '
      type(2)='*'
      PIG=3.14159265358979D0
      deliner=0.1d0
      call reset(SIM,0.d0,9*NMG)
      call reset(AL,0.d0,9)
      SIM(1,1,1)=1.d0
      SIM(2,2,1)=1.d0
      SIM(3,3,1)=1.d0
      AL(1,1)=1.d0
      AL(2,2)=1.d0
      AL(3,3)=1.d0
      F=0.d0 
      IES=0 
      N=0 
      NMS=1
c selection of group of atoms to be symmetrized
      write(7,1)MO
    1 format(//,38H           SYMMETRIZATION OF GROUP NR.,i2,//)
      do 450 i=1,NA
      call reset(AA,0.d0,9)
      AA(1,1)=SX(1,I)
      AA(2,2)=SX(2,I)
      AA(3,3)=SX(3,I)
      call prodmv(O,SX,AA,1,I,1)
      SIG(I)=0.d0
      den=0.d0
      do 435 k=1,3
        if(AA(K,K).le.0.d0)go to 435
        SIG(I)=SIG(I)+AA(k,k)
        den=den+1.d0
  435 continue
      if(den.ne.0.d0)then
       SIG(I)=SIG(I)/den
      else
       if(indtol.eq.3.or.indwgh.eq.3)then
        write(7,*)' WARNING: there is at least an atom with s.u.=0'
        write(7,*)
        write(7,*)'  CHANGE THE WEIGHTING AND TOLERANCE SCHEME!!!'
        STOP
       endif
      endif
      if(MOL(i).ne.MO)go to 450
      N=N+1
      IASU(i)=2
      IEQAT(N)=i
      do k=1,3
        XO(k,N)=X(k,i)
      enddo
      MN(N)=MSP(i)
      if(indwgh.eq.1) WEIGHT(N)=AMAS(i)
      if(indwgh.eq.2) WEIGHT(N)=1.0D0
      if(indwgh.eq.3)then
        if(SIG(i).lt..00001d0)then
          write(7,*)' ERROR: THE S.U. OF AN ATOM IS ZERO!!!'
          stop
        endif
        WEIGHT(N)=1.d0/(SIG(I)*SIG(I))
      endif
      MK(N,1)=N
  450 continue
      porig=WEIGHT(1)
      if(N.le.2)then
      write(7,*)' GROUP WITH LESS THAN THREE ATOMS'
      return
      endif
C
C     ORTHOGONALIZATION OF THE COORDINATES OF THE ATOMS TO BE SYMMETRIZED
C     CALCULATION OF THE AVERAGE DISTANCE FROM THE CENTER OF THE GROUP
C
  460 call momin(XO,WEIGHT,O,N)
c     do i=1,3
c     write(7,'(3f10.5,5x,3f10.5)')(OR(i,k),k=1,3),(OT(i,k),k=1,3)
c     enddo
C
C DM=maximum length of vectors XO(I)
C
      DM=0.d0
      DO 480 I=1,N
      call prods(XO,XO,D(I),I,I,2)
c     write(7,'(a9,i3,4f10.5)')'atom,D(I)',I,(XO(k,i),k=1,3),D(i)
      if(D(i).gt.DM)then
      DM=D(i)
      endif
  480 continue
C
      do 500 I=1,N
      DXM(I)=DCM
      if(indtol.eq.2)DXM(I)=DCM*D(I)/DM
      if(indtol.eq.3)DXM(I)=SIG(I)*DCM
  500 continue
      MDEG=0
      delr=DABS(RIN(1)-RIN(2))/RIN(1)
      IF(delr.LE.deliner)then
        MDEG=1
      endif
      delr=DABS(RIN(2)-RIN(3))/RIN(1)
      IF(delr.LE.deliner)then
        MDEG=MDEG+2
      endif
      NDEGA=MDEG+1
      nd=(MDEG+1)/2+1
      write(7,*)'PRINCIPAL INERTIA MOMENTS and DEGENERATION DEGREE'
      write(7,'(3f10.2,i10,/)')RIN,nd
C_______________________________________________________________________
C linear groups
      if(MDEG.eq.1)then
        do i=1,N
          com=DSQRT(XO(1,i)*XO(1,i)+XO(2,i)*XO(2,i))
          if(com.gt.DXM(i))go to 990
        enddo
        call reset(CO,0.d0,9)
        CO(1,1)=-1.d0
        CO(2,2)=-1.d0
        CO(3,3)= 1.d0
        call verify(XO,CO,MK,MN,MV,N)
        CO(3,3)=-1.d0
        call verify(XO,CO,MK,MN,MV,N)
        MDEG=5
        go to 2520
      endif
C_______________________________________________________________________
  990 GO TO (1020,1020,1000,2000),NDEGA
C
C SINGLE AXIS = X ROTATES THE COORDINATES SO AS TO HAVE Z AS A SINGLE AXIS
C CONSEQUENTLY MODIFICATION OF THE ORIENTATION MATRIX
C
 1000 call reset(CO,0.d0,9)
      MDEG=1
      CO(3,1)=1.d0 
      CO(1,3)=1.d0 
      CO(2,2)=-1.d0
      call r_frame(CO,XO,N)
      COM=RIN(3)
      RIN(3)=RIN(1) 
      RIN(1)=com
C SINGLE AXIS Z
 1020 IU=3
      IB=2 
      IC=1
C SEARCH OF THE AXIS OF ORDER MORD
 1100 MORD=maxorder+1
      IF(MDEG.EQ.0)MORD=3
      NASS=1
 1110 MORD=MORD-1
      IF(MORD.EQ.3.AND.MDEG.EQ.3)MORD=2
      IF(MORD.EQ.1)GO TO 1210
C GENERATION OF THE SYMMETRY MATRIX
 1120 COST=1.d0
      call reset(CO,0.d0,9)
      ROT=2.d0*PIG/dfloat(MORD)
      CA=COS(ROT)
      CB=SIN(ROT)
 1140 CO(IB,IB)=CA*COST
      CO(IC,IC)=CO(IB,IB)
      CO(IB,IC)=CB*COST
      CO(IC,IB)=-CO(IB,IC)
      CO(IU,IU)=COST
      call verify(XO,CO,MK,MN,MV,N)
c     write(7,*)' 1140 attempt with the matrix:',NINT(cost),MORD
c     write(7,'(3f9.5)')((CO(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'  NMS MORD NASS MV'
c     write(7,'(5i5)')NMS,MORD,NASS,MV
      IF(MV.EQ.1)GO TO 1150
      IF(COST.EQ.-1.d0)GO TO 1110
      COST=-1.d0
      mmez=MORD/2
      m2=2*mmez
      if(m2.ne.MORD)go to 1140
C if MORD = 2 * mmez and mmez = odd (axes -6 = 3 / m -10 = 5 / m) we have a
c reducible element therefore it is useless to test it
      m2=2*mmez/2
      if(mmez-m2.eq.1)go to 1110
      GO TO 1140
C HAS FOUND AN ELEMENT OF SYMMETRY
 1150 IF(MORD.EQ.3.OR.MORD.EQ.6)IES=1
c     write(7,*)'MDEG,MORD',MDEG,MORD
      if(MDEG.eq.1.and.MORD.gt.2)go to 1300
C SEARCH FOR SYMMETRY ELEMENTS ON NEW AXES
 1205 if(MORD.gt.2)go to 1110
 1210 NASS=NASS+1
C ON COMPLETION OF THE GROUP
      IF(NASS.EQ.4)GO TO 2500
C RECYCLING ON THE SECOND OR THIRD AXIS
      MORD=2
      I=IU 
      IU=IB 
      IB=IC 
      IC=I
      GO TO 1120
C SEARCH OF THE ROTATION TO BRING ANY ELEMENTS OF SYMMETRY TO COINCIDE WITH THE REFERENCE AXES
 1300 IF(N.le.1)then
        write(7,*)'Single atoms, not a molecule'
        return
      endif
C SELECTION OF THE ATOM TO USE TO DETERMINE THE ROTATION
      call reset(CO,0.d0,18)
      CO(3,3)=1.d0
C division into groups equidistant from the single axis
      do I=1,N
        D(I)=DSQRT(XO(1,I)*XO(1,I)+XO(2,I)*XO(2,I))
        MD(I,1)=0
        MD(I,2)=0
      enddo
      IAI=0
C IAI = NUMBER OF ENNUPLE OF ATOMS WITH DISTANCES INCLUDED IN A RANGE
C         X< D <X+DXM*.5
C MD (I, 1) = NUMBER OF THE ENNUPLA OF WHICH THE ATOM I IS PART
C MD (IAI, 2) = N FOR THE FIRST ATOM OF THE ENNUPLA WHERE N IS THE NUMBER OF ATOMS OF THE SAME
      N1=N-1
      do 1320 I=1,N1
      IF(MD(I,1).NE.0)GO TO 1320
      IAI=IAI+1
      MD(I,1)=IAI
      MD(IAI,2)=1
      J=I+1
      DO 1310 K=J,N
      IF(MN(I).NE.MN(K).or.K.eq.I.or.MD(K,1).ne.0)GO TO 1310
      IF(ABS(D(I)-D(K)).GT.0.5d0*DXM(I)) GO TO 1310
      MD(K,1)=IAI
      MD(IAI,2)=MD(IAI,2)+1
 1310 CONTINUE
 1320 CONTINUE
c search for the minimum subgroup
      JJ=10000
      do 1330 i=1,IAI
      if(MD(I,2).ge.JJ.or.MD(I,2).le.2)go to 1330
      JJ=MD(I,2)
 1330 continue
C among the minimum subgroups the most distant is chosen from the single axis
      dm=0.
      do 1350 I=1,IAI
      if(MD(I,2).ne.JJ)go to 1350
      do 1340 k=1,N
      if(MD(k,1).ne.I)go to 1340
        if(D(k).gt.dm)then
        dm=D(k)
        II=I
      endif
 1340 continue
 1350 continue
c     write(7,*)'al 1350 II,JJ',II,JJ
c     write(7,'(a5)')'MD(1)'
c     write(7,'(25i3)'),(MD(i,1),i=1,N)
c     write(7,'(a5,25i3)')'MD(2)',(MD(i,2),i=1,IAI)
      call reset(AA,0.d0,9)
      N1=N-1
      DO 1370 I1=1,N1
      IF(MD(I1,1).NE.II)GO TO 1370
      IN1=I1+1
      DO 1360 I2=IN1,N
      IF(MD(I2,1).NE.II)GO TO 1360
      call combli(XO,XO,CO,1.d0,1.d0,I1,I2,1)    
      call prodv(CO,CO,CO,3,1,2)
      cam=CO(1,2)*CO(1,2)+CO(2,2)*CO(2,2)+CO(3,2)*CO(3,2)
      if(cam.le.0.00001)go to 1360
      call norm(CO,2)
c     write(7,'(4f10.5)')CO(1,2),CO(2,2),CO(3,2)
      call prodv(CO,CO,CO,2,3,1)
      call traspo(CO,CO,1,1)
      call r_frame(CO,XO,N)
      com=1.d0
 1355 AA(1,1)=-1
      AA(2,2)=-1
      AA(3,3)= com
      call verify(XO,AA,MK,MN,MV,N)
c     write(7,*)' 1350 attempt with the matrix:'
c     write(7,'(3f9.5)')((AA(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      AA(1,1)=-1
      AA(2,2)= 1
      AA(3,3)= com
      call verify(XO,AA,MK,MN,MV,N)
c     write(7,*)' 1350 attempt with the matrix:'
c     write(7,'(3f9.5)')((AA(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      AA(1,1)= 1
      AA(2,2)=-1
      AA(3,3)= com
      call verify(XO,AA,MK,MN,MV,N)
c     write(7,*)' 1350 attempt with the matrix:'
c     write(7,'(3f9.5)')((AA(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      AA(1,1)= 0
      AA(1,2)= 1
      AA(2,1)= 1
      AA(2,2)= 0
      AA(3,3)= com
      call verify(XO,AA,MK,MN,MV,N)
c     write(7,*)' 1350 attempt with the matrix:'
c     write(7,'(3f9.5)')((AA(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      if(NMS.ge.3)go to 1110
      if(com.le.0.0)go to 1360
      com=-com
      call reset(AA,0.d0,9)
      go to 1355
C  of each of these four matrices do the equivalent with -z !!!!!
 1360 continue
 1370 continue
      go to 2500
C 
C TRIPLY DEGENERATING CASE
 2000 N1=N-1
      N2=N-2
      DO 2010 I=1,N
      MD(I,1)=0
      MD(I,2)=0
      DO 2010 J=2,NMG
 2010 MK(I,J)=0
      IAI=0
C IAI=NUMBER OF ENNUPLES OF ATOMS WITH DISTANCES IN A RANGE
C         X< D <X+DXM*.5
C MD (I, 1) = NUMBER OF THE ENNUPLA OF WHICH THE ATOM I IS PART
C MD (IAI, 2) = N FOR THE FIRST ATOM OF THE ENNUPLA WHERE N IS THE NUMBER OF ATOMS OF THE SAME
      DO 2050 I=1,N1
      IF(MD(I,1).NE.0)GO TO 2050
      IAI=IAI+1
      MD(I,1)=IAI
      MD(IAI,2)=1
      J=I+1
      DO 2040 K=J,N
      IF(MN(I).NE.MN(K).or.K.eq.I.or.MD(K,1).ne.0)GO TO 2040
      IF(ABS(D(I)-D(K)).GT.0.5d0*DXM(I)) GO TO 2040
      MD(K,1)=IAI
      MD(IAI,2)=MD(IAI,2)+1
 2040 CONTINUE
 2050 CONTINUE
c     write(7,'(A5)')'MD(1)'
c     write(7,'(20i4)')(MD(kkk,1),kkk=1,N)
c     write(7,'(a5,/,20i4)')'MD(2)',(MD(kkk,2),kkk=1,IAI)
C IF THERE ARE ATOMS ON SPECIAL LOCATIONS USE THESE TO FIND THE ROTATION MATRIX
 2100 call reset(CO,0.d0,9)
      mmd=1000
      do i=1,IAI
        if(MD(I,2).lt.mmd.and.MD(I,2).gt.1)then
          mmd=MD(I,2)
          II=i
        endif
      enddo
c     write(7,*)'II',II
c for there to be I or Ih symmetry there must be at least 12 equivalent atoms
      if(MD(II,2).eq.12.or.MD(II,2).eq.20.or.MD(II,2).eq.30.or.MD(II,2)
     *.gt.48)call icosahed(XO,WEIGHT,N,MN,MK,MD,II,MDEG,*2780)
      if(MDEG.eq.1)go to 1020
      if(NMS.ge.5)then
        MDEG=1
        call reset(AA,0.d0,9)
        write(7,'(i3,3f9.5)')(I,(XO(k,I),k=1,3),i=1,N)
        AA(1,1)= 1
        AA(2,2)= 1
        AA(3,3)=-1
        call verify(XO,AA,MK,MN,MV,N)
        AA(1,1)= 1
        AA(2,2)=-1
        AA(3,3)= 1
        call verify(XO,AA,MK,MN,MV,N)
        AA(1,1)=-1
        AA(2,2)= 1
        AA(3,3)= 1
        call verify(XO,AA,MK,MN,MV,N)
        go to 2500
      endif
C
C AXIS 3 RESEARCH ON ATOMS IN GENERAL POSITION
C NOTE: THREE EQUIVALANT ATOMS FOR A 3 AXIS MUST FORM A EQUILATERAL TRIANGLE
C II IS THE NUMBER OF ATOMS IN THE SUB-GROUP
C
 2110 CA=0.d0
C rotation matrix 3 coincident with z
      call reset(AA,0.d0,9)
      AA(3,3)=1.d0
      AA(1,1)=-.5d0
      AA(2,2)=-.5d0
      AA(2,1)=0.5d0*DSQRT(3.d0)
      AA(1,2)=-AA(2,1)
      NMS1=1
C
C     I1,I2,I3=INDICATORS OF THE THREE POSSIBLE ATOMS GENERATORS OF AXIS 3
C
      N2=N-2
      N1=N-1
      DO 2200 I1=1,N2
      IF(MD(I1,1).NE.II)GO TO 2200
      IN1=I1+1
      DO 2190 I2=IN1,N1
      IF(MD(I2,1).NE.II)GO TO 2190
      call combli(XO,XO,CO,1.d0,-1.d0,I1,I2,4)
      call prods(CO,CO,CA,4,4,2)
      IN2=I2+1
      DO 2180 I3=IN2,N
      IF(MD(I3,1).NE.II)GO TO 2180
      call combli(XO,XO,CO,1.d0,-1.d0,I1,I3,5)
      call prods(CO,CO,CB,5,5,2)
      call combli(XO,XO,CO,1.d0,-1.d0,I2,I3,6)
      call prods(CO,CO,CC,6,6,2)
      IF(DABS(CA-CB).GT.DXM(I1))GO TO 2180
      IF(DABS(CA-CC).GT.DXM(I1))GO TO 2180
      IF(DABS(CB-CC).GT.DXM(I1))GO TO 2180
      CO(1,3)=XO(1,I1)+XO(1,I2)+XO(1,I3)
      CO(2,3)=XO(2,I1)+XO(2,I2)+XO(2,I3)
      CO(3,3)=XO(3,I1)+XO(3,I2)+XO(3,I3)
      call norm(CO,3)
c     write(7,'(a15,3i4)')'I1,I2,I3',I1,I2,I3
c     write(7,'(a5,3f9.5)')'axis',(CO(kkk,3),kkk=1,3)
C saving of the possible axis 3
      CO(1,6+NMS1)=CO(1,3)
      CO(2,6+NMS1)=CO(2,3)
      CO(3,6+NMS1)=CO(3,3)
C Check that the found axis does not already coincide with z, that is
c with the first C3 axis found
      if(NMS1.gt.1)then
        call prods(CO,CO,pro,7,8,1)
        if(DABS(pro).gt.0.9d0)go to 2180
        if(pro.gt.0.d0)then
          CO(1,8)=-CO(1,8)
          CO(2,8)=-CO(2,8)
          CO(3,8)=-CO(3,8)
        endif
      endif
C definition of a second axis
      do i=1,3
        call reset(CO,0.d0,3)
        CO(I,1)=1.d0
        call prods(CO,CO,pro,1,3,1)
        if(DABS(pro).gt.0.5)go to 2150
      enddo
C check if the axis found is a C3
C rotation to bring the C3 to coincide with z
 2150 call prodv(CO,CO,CO,3,1,2)
      call norm(CO,2)
      call prodv(CO,CO,CO,2,3,1)
      call traspo(CO,CO,1,1)
      call r_frame(CO,XO,N)
      call verify(XO,AA,MK,MN,MV,N)
      NMS=1
      NMS1=NMS1+MV
c     write(7,*)' 2150 tentativo con la matrice:'
c     write(7,'(3f9.5)')((AA(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS1,MV',NMS1,MV
      call traspo(CO,CO,1,1)
      call r_frame(CO,XO,N)
      if(NMS1.eq.3)GO TO 2350
 2180 CONTINUE
 2190 CONTINUE
 2200 CONTINUE
      write(7,2)
    2 format('********************** WARNING **********************',//,
     *'       INCREASING THE TOLERANCE COULD BE USEFUL',//,
     *'*****************************************************')
c     write(7,*)'NMS1,MV',NMS1,MV
      if(NMS1.eq.1)then
C if he has already modified the weights once he does not modify them further
C but reduces the accepted tolerance between moments of inertia for the
C calculation of degeneration
        if(porig.ne.WEIGHT(1))then
          deliner=deliner*0.1d0
          go to 460
        endif
C 
C pseudodegeneration 3 without C3 axes. The possibility still exists
C that the pseudodegeneration is complete (MDEG = 0) or that there is one
C axis 4, -4.5, -5.7, -7.8, -8
C 
        write(7,*)'Weights are changed'
        write(7,*)
        do k=1,N
          WEIGHT(K)=WEIGHT(K)*(D(k)/DM)**4
c         write(7,'(i3,g10.3)')k,WEIGHT(K)
        enddo
        go to 460
      endif
C has found only one axis 3 [= CO (7)] and returns to that reference by restoring NMS definition of a second axis
      CO(1,3)=CO(1,7)
      CO(2,3)=CO(2,7)
      CO(3,3)=CO(3,7)
      do i=1,3
        call reset(CO,0.d0,3)
        CO(I,1)=1.d0
        call prods(CO,CO,pro,1,3,1)
        if(DABS(pro).gt.0.5)go to 2310
      enddo
 2310 call prodv(CO,CO,CO,3,1,2)
      call norm(CO,2)
      call prodv(CO,CO,CO,2,3,1)
      call traspo(CO,CO,1,1)
      call r_frame(CO,XO,N)
      go to 2490
C has found two axes C3 [CO (7) and CO (8)] and uses them for determine the definitive reference
 2350 call combli(CO,CO,CO,1.d0,1.d0,7,8,1)
      call norm(CO,1)
      call combli(CO,CO,CO,1.d0,-1.d0,7,8,3)
      call prodv(CO,CO,CO,3,1,2)
      call norm(CO,2)
      call prodv(CO,CO,CO,1,2,3)
      call traspo(CO,CO,1,1)
      call r_frame(CO,XO,N)
      call reset(CO,0.d0,9)
      CO(1,1)=1.d0
      CO(2,2)=DSQRT(0.5d0)
      CO(3,3)=CO(2,2)
      CO(2,3)=CO(2,2)
      CO(3,2)=-CO(2,2)
c     write(7,*)'Nuove coordinate'
c     write(7,'(i2,3f9.5)')(lll,(XO(kkk,lll),kkk=1,3),lll=1,N)
      go to 2410
C ORTONORMALIZATION OF THE ROTATION MATRIX
 2400 call norm(CO,5)
c     write(7,*)'MD'
c     write(7,'(30i4)')(MD(i,1),i=1,N)
c     write(7,'(30i4)')(MD(i,2),i=1,N)
      call prodv(CO,CO,CO,5,6,7)
      call norm(CO,7)
      call prodv(CO,CO,CO,7,5,6)
      call trasfv(CO,CO,5,1)
      call trasfv(CO,CO,6,2)
      call trasfv(CO,CO,7,3)
      call traspo(CO,CO,1,1)
C ROTATION ON NEW BACKHOE
c     write(7,*)' CO prima di r_frame'
c     write(7,'(3f10.5)')((CO(kk,ii),ii=1,3),kk=1,3)
 2410 call r_frame(CO,XO,N)
C GENERAL VERIFICATION OF TERNARY AXIS EXISTENCE
c     write(7,*)' 2410 coordinate'
c     write(7,'(i2,3f9.5)')(lll,(XO(kkk,lll),kkk=1,3),lll=1,N)
      call reset(AA,0.d0,9)
      AA(1,2)=1.d0
      AA(2,3)=1.d0
      AA(3,1)=1.d0
      call verify(XO,AA,MK,MN,MV,N)
c     write(7,*)' 2400 tentativo con la matrice:'
c     write(7,'(3f9.5)')((CO(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      NASS=1
C found a 3 (diagonal) axis now look for a 2 axis
      IU=1
      IB=2
      IC=3
      MORD=4
      call reset(CO,0.d0,9)
      CO(1,1)=-1.d0
      CO(2,2)=-1.d0
      CO(3,3)=1.d0
      call verify(XO,CO,MK,MN,MV,N)
c     write(7,*)' 2400 tentativo con la matrice:'
c     write(7,'(3f9.5)')((CO(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      if(MV.eq.1)GO TO 1120
C there is no axis 2 aligned on x: it is a pseudodegeneration
C I put myself so that axis 3 is aligned with z
      call reset(CO,0.d0,9)
      call reset(AA,0.d0,9)
      cost=dsqrt(0.5d0)
      CO(1,1)=cost
      CO(2,2)=cost
      CO(1,2)=-cost
      CO(2,1)= cost
      CO(3,3)= 1.d0
      ang=0.5d0*DACOS(-1.d0/3.d0)
      aaa=ang*180.d0/PIG
      cosa=DCOS(ang)
      sina=DSIN(ang)
      AA(2,2)=cosa
      AA(3,3)=cosa
      AA(2,3)=-sina
      AA(3,2)=sina
      AA(1,1)=1.d0
      call prodmm(AA,CO,CO,1,1,1)
      call r_frame(CO,XO,N)
 2490 NMS=1
      MORD=6
      MDEG=1
      IU=3
      IB=2
      IC=1
      write(7,5)
    5 format(
     *'***********************************************************',/,
     *'WARNING: the degeneration degree is 3 but no cubic',/,
     *'         or icosahedral group can be found.',/,
     *'IF YOU SUSPECT THE EXISTENCE OF ONE OF THEM, PLEASE CHANGE:',/,
     *'1) the weighting scheme   OR',/,
     *'2) put MOL<0 to the atoms farest from the baricenter   OR',/,
     *'3) enlarge DCM',/,
     *'***********************************************************')
      go to 1120
C SEARCH FOR THE CENTER OF SYMMETRY
 2500 II=2
      call reset(CO,0.d0,9)
      CO(1,1)=-1.d0
      CO(2,2)=-1.d0
      CO(3,3)=-1.d0
      call verify(XO,CO,MK,MN,MV,N)
c     write(7,*)' 2500 tentativo con la matrice:'
c     write(7,'(3f9.5)')((CO(kkk,lll),lll=1,3),kkk=1,3)
c     write(7,*)'NMS,MV',NMS,MV
      IF(NMS.NE.1)GO TO 2520
      write(7,3)
      RETURN 
complete the group
 2520 call compl_gr(MK,N,*4100)
C_______________________________________________________________________
C linear groups
      if(MDEG.eq.5)go to 2790
C_______________________________________________________________________
      ntest=0
      do i=1,NMS
        if(MTG(i,i).ne.1)ntest=1
      enddo
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c variation to ensure that in a C2 or C2v axis 2 coincides with z
      if(NMS.eq.2.or.NMS.eq.4)then
        nax2=0
        lax2=0
        do i=2,NMS
          itrac=NINT(SIM(1,1,i)+SIM(2,2,i)+SIM(3,3,i))
          iabst=NINT(DABS(SIM(1,1,i))+DABS(SIM(2,2,i))+DABS(SIM(3,3,i)))
          if(iabst.ne.3)go to 2740
          if(itrac.eq.-1)then
            nax2=nax2+1
            lax2=i
          endif
        enddo
        if(nax2.eq.0.or.nax2.eq.3)go to 2780
        if(nax2.eq.1)then
          do l=1,3
            if(NINT(SIM(l,l,lax2)).eq.1)go to 2730
          enddo
 2730     if(l.eq.3)go to 2780
          call reset(CO,0.d0,9)
          i=3-l
          k=6-i-l
          CO(k,l)=1.d0
          CO(l,k)=-1.d0
          CO(i,i)=1.d0
          call r_frame(CO,XO,N)
          com=RIN(3)
          RIN(3)=RIN(l)
          RIN(l)=com
        endif
        NMS=1
        go to 1020
      endif
 2740 if(ntest.eq.0)go to 2780
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C Reference optimization for MDEG> 0
 2780 if(MDEG.eq.0)go to 2790
      do i=1,NMS
       ISU(i)=i
      enddo
      call s_coor(SIM,XO,XS,CO,D,DEL,DELM,CSMT,MK,IDM,ISU,NMS,N)
      npb=0
      do k=1,NMS
       do i=1,N
        npu=npb+i
        call trasfv(XO,ppu,i,npu)
        npu=npb+MK(i,k)
        call prodmv(SIM,XS,ppo,k,i,npu)
       enddo
       npb=npb+N
      enddo
      npu=npb
      call rms_min(V)
      arms=crms(V)
c     write(7,*)'v,RV'
c     write(7,'(3f10.5)')V,RV
      call r_frame(RV,XO,N)
c Symmetry calculation for subgroups of operations
 2790 CSM(1)=0.d0
      DEV(1)=0.d0
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
c     write(7,*)'GROUP MULTIPLICATION TABLE'
c     write(7,*)
c     nst=NMS
c     nis=25
c     if(NMS.gt.24)nst=24
c     do i=1,NMS
c      write(7,'(24i3)')(MTG(i,j),j=1,nst)
c     enddo
c     write(7,*)
c     if(nst.ne.NMS)then
c      do i=1,NMS
c       write(7,'(24i3)')(MTG(i,j),j=nis,NMS)
c      enddo
c     endif
C++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
c symmetrization by subgroups of a single element (and its powers)
      do 2950 I=2,NMS
      nes=1
      ISU(1)=nes
      k=i
 2800 nes=nes+1
      ISU(nes)=k
      l=MTG(i,k)
      if(l.eq.1)go to 2900
      k=l
      go to 2800
 2900 call s_coor(SIM,XO,XS,CO,D,DEL,DELM,comt,MK,IDM,ISU,nes,N)
      DEV(I)=DELM(4)
      CSM(I)=comt
 2950 continue
c CALCULATE THE SYMMETRIZED COORDINATES FOR THE ENTIRE GROUP
 3000 do i=1,NMS
      ISU(i)=i
      enddo
      call s_coor(SIM,XO,XS,CO,D,DEL,DELM,CSMT,MK,IDM,ISU,NMS,N)
c     write(7,*)
c     write(7,*)'FRACTIONAL COORDINATES OF THE CENTRE OF AMASS'
c     write(7,'(3f12.6,/)')BARC
c     write(7,*)'ORTHOGONAL COORDINATES OF THE CENTRE OF AMASS'
c     write(7,'(3f12.6,/)')BARO
c     write(7,*)'PRINCIPAL INERTIA MOMENTS'
c     write(7,'(3f10.2,/)')RIN
      write(7,*)
      write(7,*)'   ORTHOGONALIZATION MATRIX'
      write(7,'(3f12.6)')((OT(k,i),i=1,3),k=1,3)
      write(7,*)
      write(7,*)'ATOM      ORTHOGONAL COORDINATES      VECTORS TO MAKE
     * SYMMETRICAL THE GROUP'
c     do I=1,N
c     write(7,'(24I4)')(MK(I,K),K=1,NMS)
c     enddo
C Recupero degli atomi con MOL<0
      N1=N
      do 3100 i=1,NA
      if(MOL(i).ne.-MO)go to 3100
      N1=N1+1
      IEQAT(N1)=i
      MN(N1)=MSP(i)
      MK(N1,1)=-N1
      call combli(X,BARC,XO,1.d0,-1.d0,i,1,N1)
      call prodmv(OT,XO,XO,1,N1,N1)
      XS(1,N1)=XO(1,N1)
      XS(2,N1)=XO(2,N1)
      XS(3,N1)=XO(3,N1)
 3100 continue
      NSTART=N+1
      IF(N1.eq.N)go to 3400
      do 3300 I=NSTART,N1
      do 3200 L=2,NMS
      MK(i,l)=0
      FMIN=10000.
      call prodmv(SIM,XO,V,L,I,1)
      DO 3150 K=NSTART,N1
      IF(MN(I).NE.MN(K))GO TO 3150
      call combli(XO,V,W,1.d0,-1.d0,K,1,1)
      call prods(W,W,F,1,1,2)
      if(F.le.FMIN)then
        KK=K
        FMIN=F
      endif
 3150 continue
      if(FMIN.LT.2.*DCME)then
      MK(I,L)=KK
      else
      MK(I,1)=0
      endif
 3200 continue
 3300 continue
c     do I=1,N1
c     write(7,'(24I4)')(MK(I,K),K=1,NMS)
c     enddo
      do 3350 i=NSTART,N1
      if(MK(I,1).eq.0)go to 3350
      do k=2,NMS
        call prodmv(SIM,XO,V,k,i,1)
        l=MK(i,k)
        call combli(XS,V,XS,1.d0,1.d0,l,1,l)
      enddo
 3350 continue
      do 3360 I=NSTART,N1
      if(MK(I,1).eq.0)go to 3360
        do k=1,3
          XS(k,I)=XS(k,I)/dfloat(NMS)
        enddo
 3360 continue
 3400 write(7,*)
      call asymunit(MK,IASU,N1,NMS)
      do i=1,N1
        call combli(XS,XO,delta,1.d0,-1.d0,i,i,i)
        write(7,'(a6,2x,2(3f10.5,5x))')NAME(IEQAT(I)),
     *  (XO(k,I),k=1,3),(delta(k,i),k=1,3)
      enddo
      write(7,*)
      write(7,'(5x,a34,12x,a)')'SYMMETRIZED ORTHOGONAL COORDINATES',
     *'ATOMIC R.M.S.'
      ind1=0
      ind2=0
      do I=1,N1
      comment=' '
      car=' '
      if(MK(i,1))3410,3430,3420
 3410 comment='#'
      ind1=1
 3420 car=type(IASU(I))
      go to 3440
 3430 comment='$'
      ind2=2
 3440 if(MK(i,1).gt.0)then
        write(7,'(a6,i2,2(3f9.5,2x),a1)')NAME(IEQAT(I)),
     *  MOL(IEQAT(I)),(XS(k,I),k=1,3),(RMS(k,I),k=1,3),car
      else
        write(7,'(a6,i2,3f9.5,29x,a1,1x,a1)')NAME(IEQAT(I)),
     *  MOL(IEQAT(I)),(XS(k,I),k=1,3),comment,car
      endif
C IASU(I)=2 se l'atomo rappresenta l'unita' asimmetrica altrimenti IASU(I)=1
      enddo
      ind1=ind1+ind2
      write(7,*)
      write(7,*)'* Atom defining the asymmetric unit for the found sym
     *metry group.'
      if(ind1.eq.0)go to 3460
      if(ind1.eq.2)go to 3450
      write(7,*)
      write(7,*)'# This atom was symmetrized but NOT used to find the
     *symmetry group and to calculate CMS, RMS and so on.'
 3450 if(ind1.eq.1)go to 3460
       write(7,*)
       write(7,*)'$ It was IMPOSSIBLE to symmetrize this atom accordin
     *g to the found symmetry group and within the given tolerance.'
 3460 write(7,*)
      do i=1,4
        DEL(i)=DEL(I)/dfloat(N)
        id1=idm(i)
        if(IDM(i).ne.0) then
        ie1=IEQAT(id1)
          NNAME(i)=NAME(ie1)
        else
          NNAME(i)=' --- '
        endif
      enddo
      write(7,*)
      write(7,'(1x,a29,4F10.5,/)')'AVERAGE DIFFERENCE ON X,Y,Z,D',DEL
      write(7,'(1x,a29,4F10.5)')'MAXIMUM DIFFERENCE ON X,Y,Z,D',DELM
      write(7,'(1x,a29,4a10,/)')'DUE TO THE ATOMS',
     *(NNAME(i),i=1,4)
C
      do i=1,N 
      do j=1,NMS
      K=MK(i,j)
      call prodmv(SIM,XS,V,j,i,1)
      do l=1,3
      diff=abs(XS(l,k)-V(l))
      if(diff.gt.0.001d0)then
       write(7,*)
       write(7,*)
     * '                    ========> CAUTION!!! <=======' 
       write(7,*)
       write(7,*)  ' THE SYMMETRY OPERATIONS ARE NOT CONSISTENT WITH THE
     * EQUIVALENTS POINTS.'
       write(7,'(a27,f10.5)')' DIFFERENCE FOUND =',diff
       write(7,*)
     * '     ========> TRY TO REDUCE THE CONSTANT OF TOLERANCE <=======' 
c      write(7,'(3(3f11.7,2x))')(XS(kk,i),kk=1,3),V,(XS(kk,k),kk=1,3)
       go to 3500
c      RETURN
      endif
      enddo
      enddo
      enddo
C 
 3500 write(7,*)'Bond lengths and bond angles after symmetrization'
      call bond(XS,IASU,N1)
      call out_bond(XS,IEQAT,IASU,N)
      if(PC(7).gt.2.d0)then
      write(7,'(7x,a)')' SYMMETRIZED FRACTIONAL COORDINATES'
      do 3600 i=1,N1
      k=IEQAT(i)
      call prodmv(OTI,XS,X,1,i,k)
      call combli(X,BARC,X,1.d0,1.d0,k,1,k)
      write(7,'(1x,a6,i2,6f9.5)')NAME(k),MOL(k),(X(j,k),j=1,3),
     *(SX(j,k),j=1,3)
 3600 continue
      endif
      call schoenfl(MDEG)
      if(IPOGROU.ne.0.and.(IPOGROU.lt.16.or.IPOGROU.gt.27))then
      call stasy
      else
      write(7,6)
      do 3800 k=1,NMS
      write(7,8)k,CSM(k),DEV(k),lbls(K)
    8 format(/,i3,' CSM =',f7.2,5x,'MAX. DIFF. (Angstrom)=',F6.4,
     *5x,'TYPE ',a3)
      write(7,7)((SIM(I,J,K),J=1,3),I=1,3)
 3800 continue
      endif
      IF(IES.NE.1)RETURN
      if(IPOGROU.le.0.or.IPOGROU.gt.32)RETURN
      AL(2,2)=SQRT(3.D0)*0.5D0
      AL(1,2)=-0.5D0
      AL(1,1)=1.d0
      AL(3,3)=1.d0
      call inv33(AL,ALX,1,1)
      write(7,*)' SYMMETRY OPERATIONS IN HEXAGONAL COORDINATES'
      DO 3900 I=1,NMS
      call prodmm(ALX,SIM,AA,1,I,1)
      call prodmm(AA,AL,SIM,1,1,I)
 3900 continue
      call stasy
      write(7,*)
      write(7,11)
   11 format(' OBLIQUE COORDINATES (HEXAGONAL SYSTEM)')
      do 4000 I=1,N1
      call prodmv(ALX,XS,CO,1,I,I)
      write(7,'(1x,a8,3f10.5)')NAME(IEQAT(I)),(CO(J,I),J=1,3)
 4000 continue
 4100 RETURN 
    3 format(' NO SYMMETRY EXISTS WITHIN GIVEN TOLERANCE')
    6 format(/,' SYMMETRY GROUP MATRICES',/)
    7 format(3(1X,3F16.10,/))
      close(7)
      END
